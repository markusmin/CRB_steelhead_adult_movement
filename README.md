...
Repository for the following publication: Min, M. A., Buchanan, R. A., & Scheuerell, M.D. (2025). Modeling climate and hydropower influences on the movement decisions of an anadromous species. Global Change Biology.
...
The scripts can all be found in the `Stan/` folder. One subfolder exists for each combination of DPS/rear type (and juvenile pathway, in the case of Snake River fish). Additionally, the `Stan/alternative_spill_treatment` folder contains the code to re-run all of the same models, but with only the effect of March spill (rather than Winter spill). In each subfolder, you will find the following files:
...
the model output files are quite large, many of these scripts were run on Hyak. Please 
note the final model analysis scripts are found in both the `/transport/` subfolder of
the `R/07_model_analysis` folder as well as in the main folder.
...
- `/07-07_final_fates_scenarios/`
  ...
  - `/transport/07-07_final_fates_scenarios/` also contains the jobs re-run with
  Tucannon River transported and in-river (not transported) fish modeled separately.
...
- `Min_2025_supplement.Rmd` this contains all of the scripts necessary to reproduce
