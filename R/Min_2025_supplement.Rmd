...
```{r, fig.cap = "Residence time for fish in the state between Bonneville and McNary Dams."}
BON_MCN_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state between McNary and Ice Harbor/Priest Rapids Dams."}
MCN_ICH_PRA_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state between Priest Rapids and Rock Island Dams."}
PRA_RIS_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state between Rock Island and Rocky Reach Dams."}
RIS_RRE_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state between Rocky Reach and Wells Dams."}
RRE_WEL_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state between Ice Harbor and Lower Granite Dams."}
ICH_LGR_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state upstream of Wells Dam."}
upstream_WEL_plot
```

```{r, fig.dim = c(5,5), fig.cap = "Residence time for fish in the state upstream of Lower Granite Dam."}
upstream_LGR_plot
```
...
extract_run_year_origin <- function(data, rear, transport_type, DPS, origin_param_map = origin_param_map){
  ...
  # add the juvenile pathway column
  fish_state_origin_year$juvenile_pathway <- transport_type
...
UCW_origin_run_years <- extract_run_year_origin(data = UCW_data, rear = "natural", transport_type = "in-river",
                                                DPS = "Upper Columbia", origin_param_map = origin_param_map)
UCH_origin_run_years <- extract_run_year_origin(data = UCH_data, rear = "hatchery", transport_type = "in-river",
                                                DPS = "Upper Columbia", origin_param_map = origin_param_map)
MCW_origin_run_years <- extract_run_year_origin(data = MCW_data, rear = "natural", transport_type = "in-river",
                                                DPS = "Middle Columbia", origin_param_map = origin_param_map)
MCH_origin_run_years <- extract_run_year_origin(data = MCH_data, rear = "hatchery", transport_type = "in-river",
                                                DPS = "Middle Columbia", origin_param_map = origin_param_map)
SRW_T_origin_run_years <- extract_run_year_origin(data = SRW_T_data, rear = "natural", transport_type = "transported",
                                                DPS = "Snake River", origin_param_map = origin_param_map)
SRW_NT_origin_run_years <- extract_run_year_origin(data = SRW_NT_data, rear = "natural", transport_type = "in-river",
                                                DPS = "Snake River", origin_param_map = origin_param_map)
SRH_T_origin_run_years <- extract_run_year_origin(data = SRH_T_data, rear = "hatchery", transport_type = "transported",
                                                DPS = "Snake River", origin_param_map = origin_param_map)
SRH_NT_origin_run_years <- extract_run_year_origin(data = SRH_NT_data, rear = "hatchery", transport_type = "in-river",
                                                DPS = "Snake River", origin_param_map = origin_param_map)
...
UCW_origin_run_years %>% 
  ...
  bind_rows(., SRW_T_origin_run_years) %>% 
  bind_rows(., SRW_NT_origin_run_years) %>% 
  bind_rows(., SRH_T_origin_run_years) %>% 
  bind_rows(., SRH_NT_origin_run_years) -> full_origin_run_years

full_origin_run_years %>% 
  group_by(natal_origin, run_year, rear, juvenile_pathway) %>% 
...
sample_size_long %>% 
  ...
  mutate(population = paste0(natal_origin, " (", ifelse(rear == "natural", "N, ", "H, "), juvenile_pathway, ")")) %>% 
  dplyr::select(-c(natal_origin, rear, juvenile_pathway)) %>% 
...
sample_size_table %>% 
  ...
  kbl(caption = "The number of fish per run year from each combination of natal tributary, rearing type (natural or hatchery origin), and juvenile pathway (transported or in-river migrants).") %>% 
...
plot_covariates_one_dam <- function(data, dam_abbrev){
  ...
  scale_x_date(date_breaks = "1 month", date_labels = c("Jun 1", "Jul 1", "Aug 1", 
                                                        "Sep 1", "Oct 1", "Nov 1", 
                                                        "Dec 1", "Jan 1", "Feb 1", 
                                                        "Mar 1", "Apr 1", "May 1")) +
...
all_dams_temp_table %>% 
  ...
  kbl(caption = "Mean temperature (degrees Celsius) by run year across the dams in this study. Dam abbreviations are the ones listed in Table 1 of the main text.") %>% 
  kable_styling(latex_options = "HOLD_position")

all_dams_spill_table %>% 
  mutate(across(where(is.numeric), ~ round(., 2))) %>% 
  dplyr::rename("Run Year" = run_year) %>% 
  kbl(caption = "Mean spill volume (kcfs) by run year across the dams in this study. Dam abbreviations are the ones listed in Table 1 of the main text.") %>% 
  kable_styling(latex_options = "HOLD_position")
...
winter_spill_days_table %>% 
  relocate(run_year) %>% 
  dplyr::rename("Run Year" = run_year) %>% 
  kbl(caption = "Number of winter (January - March) spill days by run year across the dams in this study. Dam abbreviations are the ones listed in Table 1 of the main text. Please note that Bonneville Dam is not included in this plot because it is not an overshoot dam for any populations in our study.") %>% 
  kable_styling(latex_options = "HOLD_position")
...
### 3 Post-overshoot fallback timing {.tabset .tabset-pills}
...
plot_overshoot_fallback_timing <- function(data, natal_origin_select, DPS_select, rear_select, transport_type_select){
...
overshoot_fallback_timing_plot <- ggplot(origin_end_overshoot_history, aes(y = ID_new)) +
  ...
  scale_x_date(date_breaks = "1 month", date_labels = c("Jun 1", "Jul 1", "Aug 1", 
    ...
    theme(legend.position = "none",
          ...
          axis.ticks.y = element_blank(),
          plot.margin = unit(c(0.2, 0.2, 0.2, 0.2),"cm")) +
  ggtitle(paste0(natal_origin_select, ", ", ifelse(rear_select == "wild", "natural", rear_select), ", ", transport_type_select))
...
as.data.frame(table(origin_end_overshoot_history$jan_mar_window)) %>% 
  ...
  mutate(population = paste(natal_origin_select, rear_select, transport_type_select)) %>% 
...
DES_W_overshoot_fallback_timing_plot <- plot_overshoot_fallback_timing(data = MCW_data, 
                               ...
                               rear_select = "wild",
                               transport_type_select = "in-river")
...
TUC_W_T_overshoot_fallback_timing_plot <- plot_overshoot_fallback_timing(data = SRW_T_data, 
                               natal_origin_select = "Tucannon River", 
                               DPS_select = "Snake River", 
                               rear_select = "wild",
                               transport_type_select = "transported")

TUC_W_NT_overshoot_fallback_timing_plot <- plot_overshoot_fallback_timing(data = SRW_NT_data, 
                               natal_origin_select = "Tucannon River", 
                               DPS_select = "Snake River", 
                               rear_select = "wild",
                               transport_type_select = "in-river")

TUC_H_T_overshoot_fallback_timing_plot <- plot_overshoot_fallback_timing(data = SRH_T_data, 
                               natal_origin_select = "Tucannon River", 
                               DPS_select = "Snake River", 
                               rear_select = "hatchery",
                               transport_type_select = "transported")

TUC_H_NT_overshoot_fallback_timing_plot <- plot_overshoot_fallback_timing(data = SRH_NT_data, 
                               natal_origin_select = "Tucannon River", 
                               DPS_select = "Snake River", 
                               rear_select = "hatchery",
                               transport_type_select = "in-river")
...
```{r fallback_timing_all_origin_summaries, fig.dim = c(8,8), fig.cap = "Timing of detections directly following a post-overshoot fallback movement."}
...
DES_W_overshoot_fallback_timing_plot[[4]] %>% 
  ...
  bind_rows(TUC_W_T_overshoot_fallback_timing_plot[[4]]) %>% 
  bind_rows(TUC_W_NT_overshoot_fallback_timing_plot[[4]]) %>% 
  bind_rows(TUC_H_T_overshoot_fallback_timing_plot[[4]]) %>%
  bind_rows(TUC_H_NT_overshoot_fallback_timing_plot[[4]]) -> known_fallback_timing_all_origins

 population_colors <- c("Deschutes River natural in-river" = "#30123BFF",
                         "John Day River natural in-river" = "#4147ADFF", 
                         "Fifteenmile Creek natural in-river" = "#4777EFFF",
                         "Umatilla River natural in-river" = "#39A4FBFF", 
                         "Umatilla River hatchery in-river" =  "#1BD0D5FF", 
                         "Yakima River natural in-river" =  "#25ECA7FF", 
                         "Walla Walla River natural in-river" = "#62FC6BFF", 
                         "Walla Walla River hatchery in-river" = "#A2FC3CFF", 
                         "Wenatchee River natural in-river" = "#D2E935FF", 
                         "Wenatchee River hatchery in-river" = "#F4C73AFF", 
                         "Entiat River natural in-river" = "#FE9B2DFF",
  ...
  known_fallback_timing_all_origins$population <- factor(known_fallback_timing_all_origins$population, 
                                                         levels = c("Deschutes River natural in-river",
                                                                    "John Day River natural in-river", 
                                                                    "Fifteenmile Creek natural in-river",
                         "Umatilla River natural in-river", 
                         "Umatilla River hatchery in-river", 
                         "Yakima River natural in-river", 
                         "Walla Walla River natural in-river", 
                         "Walla Walla River hatchery in-river", 
                         "Wenatchee River natural in-river", 
                         "Wenatchee River hatchery in-river", 
                         "Entiat River natural in-river",
...
DES_W_overshoot_fallback_timing_plot[[2]] %>% 
  ...
  bind_rows(TUC_W_NT_overshoot_fallback_timing_plot[[2]]) %>% 
  bind_rows(TUC_W_T_overshoot_fallback_timing_plot[[2]]) %>% 
  bind_rows(TUC_H_NT_overshoot_fallback_timing_plot[[2]]) %>% 
  bind_rows(TUC_H_T_overshoot_fallback_timing_plot[[2]]) %>%
...
![Tucannon River natural origin, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/tuc_wild_ff_transport_comp_median_plot.png){width=100%}

![Tucannon River hatchery, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/tuc_hatchery_ff_transport_comp_median_plot.png){width=100%}
...
![Clearwater River natural origin, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/cle_wild_ff_transport_comp_median_plot.png){width=100%}

![Clearwater River hatchery, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/cle_hatchery_ff_transport_comp_median_plot.png){width=100%}

#### Asotin Creek
![Asotin Creek natural origin, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/aso_wild_ff_transport_comp_median_plot.png){width=100%}
...
![Grande Ronde natural origin, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/gr_wild_ff_transport_comp_median_plot.png){width=100%}

![Grande Ronde hatchery, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/gr_hatchery_ff_transport_comp_median_plot.png){width=100%}
...
![Salmon River natural origin, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/sal_wild_ff_transport_comp_median_plot.png){width=100%}

![Salmon River hatchery, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/sal_hatchery_ff_transport_comp_median_plot.png){width=100%}

#### Imnaha River
![Imnaha River natural origin, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/imn_wild_ff_transport_comp_median_plot.png){width=100%}

![Imnaha River hatchery, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/imn_hatchery_ff_transport_comp_median_plot.png){width=100%}
![Imnaha River, non-transported fish, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/imn_not_transported_ff_rear_comp_median_plot.png){width=100%}

![Imnaha River, transported fish, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/imn_transported_ff_rear_comp_median_plot.png){width=100%}

![Imnaha River, natural origin fish, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/imn_wild_ff_transport_comp_median_plot.png){width=100%}

![Imnaha River, hatchery origin fish, estimated final fates under median conditions from 2005-2024.](/Users/markusmin/Documents/CBR/CRB_steelhead_adult_movement/figures/transport/final_fates/imn_hatchery_ff_transport_comp_median_plot.png){width=100%}
...
```{r}
all_overshoot_summary_table <- read.csv(here::here("figures", "transport", "final_fates", "all_overshoot_summary_table.csv"), row.names = 1)
all_overshoots_homing_summary_table <- read.csv(here::here("figures", "transport", "final_fates", "all_overshoots_homing_summary_table.csv"), row.names = 1)
all_overshoots_homing_conditional_summary_table <- read.csv(here::here("figures", "transport", "final_fates", "all_overshoots_homing_conditional_summary_table.csv"), row.names = 1)


# re-arrange the tables
population_order <- c("John Day River, Natural",
                      ...
                      "Tucannon River, Natural (not transported)",
                      "Tucannon River, Natural (transported)",
                      "Tucannon River, Hatchery (not transported)",
                      "Tucannon River, Hatchery (transported)")

all_overshoot_summary_table$origin_rear <- factor(all_overshoot_summary_table$origin_rear, levels = population_order)
all_overshoots_homing_summary_table$origin_rear <- factor(all_overshoots_homing_summary_table$origin_rear, levels = population_order)
all_overshoots_homing_conditional_summary_table$origin_rear <- factor(all_overshoots_homing_conditional_summary_table$origin_rear, levels = population_order)

colnames(all_overshoot_summary_table) <- c("Population", "No overshoot", "1", "2+")
colnames(all_overshoots_homing_summary_table) <- c("Population", "No overshoot", "1", "2+")
colnames(all_overshoots_homing_conditional_summary_table) <- c("Population", "No overshoot", "1", "2+")

all_overshoot_summary_table %>% 
  arrange(Population) %>% 
  dplyr::select(-`No overshoot`) %>% 
  kbl(caption = "Probability (with 95% credible interval) of overshooting one or two or more dams by population. The Entiat River and Tucannon River populations are only able to overshoot one dam in our model (Wells Dam and Lower Granite Dam, respectively).") %>% 
  kable_styling(latex_options = "HOLD_position")

all_overshoots_homing_conditional_summary_table %>% 
  arrange(Population) %>% 
  kbl(caption = "Probability (with 95% credible interval) of homing, conditional on ascending the dam directly downstream of the natal tributary but not overshooting (No overshoot), overshooting one dam (1), or overshooting two or more dams (2+) by population. The Entiat River and Tucannon River populations are only able to overshoot one dam in our model (Wells Dam and Lower Granite Dam, respectively).") %>% 
  kable_styling(latex_options = "HOLD_position")
