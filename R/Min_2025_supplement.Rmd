...
subtitle: "Climate and hydropower interact to determine migration success of adult Steelhead in the Columbia River Basin"
...
### 2 Covariate correlations {.tabset .tabset-pills}
...
origin_end_overshoot_history %>% 
  ...
  mutate(plotting_date = ymd(paste0(plotting_year, "-", month_day))) -> origin_end_overshoot_history

origin_end_overshoot_history$ID_new <- rep(1:(nrow(origin_end_overshoot_history)/2), each = 2)
...
overshoot_fallback_timing_plot <- ggplot(origin_end_overshoot_history, aes(x = plotting_date, y = ID_new)) +
  geom_line(aes(group = ID_new), linewidth = 0.25) +
  geom_point() +
  geom_point(data = subset(origin_overshoot_loss, state != 41), aes(x = plotting_date, y = ID_new), color = "red") +
...
return(overshoot_fallback_timing_plot)
...
#### Deschutes (N)
```{r fig.dim = c(6,10)}
DES_W_overshoot_fallback_timing_plot
```

#### John Day (N)
```{r fig.dim = c(6,10)}
JDR_W_overshoot_fallback_timing_plot
```

#### Fifteenmile (N)
```{r fig.dim = c(6,10)}
FIF_W_overshoot_fallback_timing_plot
```

#### Umatilla (N)
```{r fig.dim = c(6,10)}
UMA_W_overshoot_fallback_timing_plot
```

#### Umatilla (H)
```{r fig.dim = c(6,10)}
UMA_H_overshoot_fallback_timing_plot
```

#### Yakima (N)
```{r fig.dim = c(6,10)}
YAK_W_overshoot_fallback_timing_plot
```

#### Walla Walla (N)
```{r fig.dim = c(6,10)}
WAWA_W_overshoot_fallback_timing_plot
```

#### Walla Walla (H)
```{r fig.dim = c(6,10)}
WAWA_H_overshoot_fallback_timing_plot
```

#### Entiat (N)
```{r fig.dim = c(6,10)}
ENT_W_overshoot_fallback_timing_plot
```

#### Wenatchee (N)
```{r fig.dim = c(6,10)}
WEN_W_overshoot_fallback_timing_plot
```

#### Wenatchee (H)
```{r fig.dim = c(6,10)}
WEN_H_overshoot_fallback_timing_plot
```

#### Tucannon (N)
```{r fig.dim = c(6,10)}
TUC_W_overshoot_fallback_timing_plot
```

#### Tucannon (H)
```{r fig.dim = c(6,10)}
TUC_H_overshoot_fallback_timing_plot
```
...
### 8 Spill volume vs. fallback rates {.tabset .tabset-pills}
...
### 9 Post-overshoot fallback vs. March spill
