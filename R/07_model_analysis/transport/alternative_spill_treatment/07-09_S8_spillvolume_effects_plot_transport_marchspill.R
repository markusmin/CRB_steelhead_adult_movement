...
estimate_spillwindow_effect_SRW_T <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    # set up spill values to predict across
    # extract the data
    spill_window_data <- SRW_T_envir$data$spill_window_data
    # create a sequence of values from 1 to max
    spill_predict <- seq(0, max(spill_window_data[,from]),length = 100)
    
    # Get the movements
    possible_movements <- SRW_T_envir$data$movements[, "col"][SRW_T_envir$data$movements[, "row"] == from]
    possible_movements <- c(possible_movements, 41)
    ...
    SRW_T_states_dates <- data.frame(state = as.vector(SRW_T_envir$data$y),
                                     date = as.vector(SRW_T_envir$data$transition_dates))
    
    # get median temperature for this state
    temp_data <- SRW_T_envir$data$temperature
    med_temp <- median(temp_data[subset(SRW_T_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_predict)){
        # evaluation movement 
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRW_T[movements$from[i],movements$to[i],iter] +
                                                 # btemp0_array_SRW_T[movements$from[i],movements$to[i],iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                 btemp1_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp + 
                                                 bspillwindow_array_SRW_T[movements$from[i],movements$to[i],iter]*spill_predict[j] +
                                                 # bwinterspill_array_SRW_T[movements$from[i],movements$to[i],iter]*med_winterspill + # indexing is set up so that if spill window is selected, spill days is ignored
                                                 # btemp0xorigin1_array_SRW_T[movements$from[i],movements$to[i],iter]*origin1 +
                                                 btemp1xorigin1_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp0xorigin2_array_SRW_T[movements$from[i],movements$to[i],iter]*origin2 + 
                                                 btemp1xorigin2_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 # btemp0xorigin3_array_SRW_T[movements$from[i],movements$to[i],iter]*origin3 +
                                                 btemp1xorigin3_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp0xorigin4_array_SRW_T[movements$from[i],movements$to[i],iter]*origin4 +
                                                 btemp1xorigin4_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp0xorigin5_array_SRW_T[movements$from[i],movements$to[i],iter]*origin5 +
                                                 btemp1xorigin5_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 # btemp0xorigin6_array_SRW_T[movements$from[i],movements$to[i],iter]*origin6 +
                                                 btemp1xorigin6_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin6 +
                                                 borigin1_array_SRW_T[movements$from[i],movements$to[i],iter]*origin1 +
                                                 borigin2_array_SRW_T[movements$from[i],movements$to[i],iter]*origin2 +
                                                 borigin3_array_SRW_T[movements$from[i],movements$to[i],iter]*origin3 +
                                                 borigin4_array_SRW_T[movements$from[i],movements$to[i],iter]*origin4 +
                                                 borigin5_array_SRW_T[movements$from[i],movements$to[i],iter]*origin5 +
                                                 borigin6_array_SRW_T[movements$from[i],movements$to[i],iter]*origin6)/
          sum(exp(b0_array_SRW_T[movements$from[i],possible_movements,iter] +
                    # btemp0_array_SRW_T[movements$from[i],possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp + 
                    bspillwindow_array_SRW_T[movements$from[i],possible_movements,iter]*spill_predict[j] +
                    # bwinterspill_array_SRW_T[movements$from[i],possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRW_T[movements$from[i],possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp0xorigin2_array_SRW_T[movements$from[i],possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    # btemp0xorigin3_array_SRW_T[movements$from[i],possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp0xorigin4_array_SRW_T[movements$from[i],possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp0xorigin5_array_SRW_T[movements$from[i],possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    # btemp0xorigin6_array_SRW_T[movements$from[i],possible_movements,iter]*origin6 +
                    btemp1xorigin6_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin6 +
                    borigin1_array_SRW_T[movements$from[i],possible_movements,iter]*origin1 +
                    borigin2_array_SRW_T[movements$from[i],possible_movements,iter]*origin2 +
                    borigin3_array_SRW_T[movements$from[i],possible_movements,iter]*origin3 +
                    borigin4_array_SRW_T[movements$from[i],possible_movements,iter]*origin4 +
                    borigin5_array_SRW_T[movements$from[i],possible_movements,iter]*origin5 +
                    borigin6_array_SRW_T[movements$from[i],possible_movements,iter]*origin6))
...
estimate_spillwindow_effect_SRW_NT <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    # set up spill values to predict across
    # extract the data
    spill_window_data <- SRW_NT_envir$data$spill_window_data
    # create a sequence of values from 1 to max
    spill_predict <- seq(0, max(spill_window_data[,from]),length = 100)
    
    # Get the movements
    possible_movements <- SRW_NT_envir$data$movements[, "col"][SRW_NT_envir$data$movements[, "row"] == from]
    possible_movements <- c(possible_movements, 41)
    ...
    SRW_NT_states_dates <- data.frame(state = as.vector(SRW_NT_envir$data$y),
                                      date = as.vector(SRW_NT_envir$data$transition_dates))
    
    # get median temperature for this state
    temp_data <- SRW_NT_envir$data$temperature
    med_temp <- median(temp_data[subset(SRW_NT_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_predict)){
        # evaluation movement 
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRW_NT[movements$from[i],movements$to[i],iter] +
                                                 # btemp0_array_SRW_NT[movements$from[i],movements$to[i],iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                 btemp1_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp + 
                                                 bspillwindow_array_SRW_NT[movements$from[i],movements$to[i],iter]*spill_predict[j] +
                                                 # bwinterspill_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_winterspill + # indexing is set up so that if spill window is selected, spill days is ignored
                                                 # btemp0xorigin1_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin1 +
                                                 btemp1xorigin1_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp0xorigin2_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin2 + 
                                                 btemp1xorigin2_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 # btemp0xorigin3_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin3 +
                                                 btemp1xorigin3_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp0xorigin4_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin4 +
                                                 btemp1xorigin4_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp0xorigin5_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin5 +
                                                 btemp1xorigin5_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 # btemp0xorigin6_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin6 +
                                                 btemp1xorigin6_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin6 +
                                                 borigin1_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin1 +
                                                 borigin2_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin2 +
                                                 borigin3_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin3 +
                                                 borigin4_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin4 +
                                                 borigin5_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin5 +
                                                 borigin6_array_SRW_NT[movements$from[i],movements$to[i],iter]*origin6)/
          sum(exp(b0_array_SRW_NT[movements$from[i],possible_movements,iter] +
                    # btemp0_array_SRW_NT[movements$from[i],possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp + 
                    bspillwindow_array_SRW_NT[movements$from[i],possible_movements,iter]*spill_predict[j] +
                    # bwinterspill_array_SRW_NT[movements$from[i],possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRW_NT[movements$from[i],possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp0xorigin2_array_SRW_NT[movements$from[i],possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    # btemp0xorigin3_array_SRW_NT[movements$from[i],possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp0xorigin4_array_SRW_NT[movements$from[i],possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp0xorigin5_array_SRW_NT[movements$from[i],possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    # btemp0xorigin6_array_SRW_NT[movements$from[i],possible_movements,iter]*origin6 +
                    btemp1xorigin6_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin6 +
                    borigin1_array_SRW_NT[movements$from[i],possible_movements,iter]*origin1 +
                    borigin2_array_SRW_NT[movements$from[i],possible_movements,iter]*origin2 +
                    borigin3_array_SRW_NT[movements$from[i],possible_movements,iter]*origin3 +
                    borigin4_array_SRW_NT[movements$from[i],possible_movements,iter]*origin4 +
                    borigin5_array_SRW_NT[movements$from[i],possible_movements,iter]*origin5 +
                    borigin6_array_SRW_NT[movements$from[i],possible_movements,iter]*origin6))
...
estimate_spillwindow_effect_SRH_T <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    # set up spill values to predict across
    # extract the data
    spill_window_data <- SRH_T_envir$data$spill_window_data
    # create a sequence of values from 1 to max
    spill_predict <- seq(0, max(spill_window_data[,from]),length = 100)
    
    # Get the movements
    possible_movements <- SRH_T_envir$data$movements[, "col"][SRH_T_envir$data$movements[, "row"] == from]
    possible_movements <- c(possible_movements, 41)
    ...
    SRH_T_states_dates <- data.frame(state = as.vector(SRH_T_envir$data$y),
                                     date = as.vector(SRH_T_envir$data$transition_dates))
    
    # get median temperature for this state
    temp_data <- SRH_T_envir$data$temperature
    med_temp <- median(temp_data[subset(SRH_T_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_predict)){
        # evaluation movement 
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRH_T[movements$from[i],movements$to[i],iter] +
                                                 # btemp0_array_SRH_T[movements$from[i],movements$to[i],iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                 btemp1_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp + 
                                                 bspillwindow_array_SRH_T[movements$from[i],movements$to[i],iter]*spill_predict[j] +
                                                 # bwinterspill_array_SRH_T[movements$from[i],movements$to[i],iter]*med_winterspill + # indexing is set up so that if spill window is selected, spill days is ignored
                                                 # btemp0xorigin1_array_SRH_T[movements$from[i],movements$to[i],iter]*origin1 +
                                                 btemp1xorigin1_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp0xorigin2_array_SRH_T[movements$from[i],movements$to[i],iter]*origin2 + 
                                                 btemp1xorigin2_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 # btemp0xorigin3_array_SRH_T[movements$from[i],movements$to[i],iter]*origin3 +
                                                 btemp1xorigin3_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp0xorigin4_array_SRH_T[movements$from[i],movements$to[i],iter]*origin4 +
                                                 btemp1xorigin4_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp0xorigin5_array_SRH_T[movements$from[i],movements$to[i],iter]*origin5 +
                                                 btemp1xorigin5_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 borigin1_array_SRH_T[movements$from[i],movements$to[i],iter]*origin1 +
                                                 borigin2_array_SRH_T[movements$from[i],movements$to[i],iter]*origin2 +
                                                 borigin3_array_SRH_T[movements$from[i],movements$to[i],iter]*origin3 +
                                                 borigin4_array_SRH_T[movements$from[i],movements$to[i],iter]*origin4 +
                                                 borigin5_array_SRH_T[movements$from[i],movements$to[i],iter]*origin5)/
          sum(exp(b0_array_SRH_T[movements$from[i],possible_movements,iter] +
                    # btemp0_array_SRH_T[movements$from[i],possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp + 
                    bspillwindow_array_SRH_T[movements$from[i],possible_movements,iter]*spill_predict[j] +
                    # bwinterspill_array_SRH_T[movements$from[i],possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRH_T[movements$from[i],possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp0xorigin2_array_SRH_T[movements$from[i],possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    # btemp0xorigin3_array_SRH_T[movements$from[i],possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp0xorigin4_array_SRH_T[movements$from[i],possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp0xorigin5_array_SRH_T[movements$from[i],possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    borigin1_array_SRH_T[movements$from[i],possible_movements,iter]*origin1 +
                    borigin2_array_SRH_T[movements$from[i],possible_movements,iter]*origin2 +
                    borigin3_array_SRH_T[movements$from[i],possible_movements,iter]*origin3 +
                    borigin4_array_SRH_T[movements$from[i],possible_movements,iter]*origin4 +
                    borigin5_array_SRH_T[movements$from[i],possible_movements,iter]*origin5))
...
estimate_spillwindow_effect_SRH_NT <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    # set up spill values to predict across
    # extract the data
    spill_window_data <- SRH_NT_envir$data$spill_window_data
    # create a sequence of values from 1 to max
    spill_predict <- seq(0, max(spill_window_data[,from]),length = 100)
    
    # Get the movements
    possible_movements <- SRH_NT_envir$data$movements[, "col"][SRH_NT_envir$data$movements[, "row"] == from]
    possible_movements <- c(possible_movements, 41)
    ...
    SRH_NT_states_dates <- data.frame(state = as.vector(SRH_NT_envir$data$y),
                                      date = as.vector(SRH_NT_envir$data$transition_dates))
    
    # get median temperature for this state
    temp_data <- SRH_NT_envir$data$temperature
    med_temp <- median(temp_data[subset(SRH_NT_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_predict)){
        # evaluation movement 
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRH_NT[movements$from[i],movements$to[i],iter] +
                                                 # btemp0_array_SRH_NT[movements$from[i],movements$to[i],iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                 btemp1_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp + 
                                                 bspillwindow_array_SRH_NT[movements$from[i],movements$to[i],iter]*spill_predict[j] +
                                                 # bwinterspill_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_winterspill + # indexing is set up so that if spill window is selected, spill days is ignored
                                                 # btemp0xorigin1_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin1 +
                                                 btemp1xorigin1_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp0xorigin2_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin2 + 
                                                 btemp1xorigin2_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 # btemp0xorigin3_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin3 +
                                                 btemp1xorigin3_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp0xorigin4_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin4 +
                                                 btemp1xorigin4_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp0xorigin5_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin5 +
                                                 btemp1xorigin5_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 borigin1_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin1 +
                                                 borigin2_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin2 +
                                                 borigin3_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin3 +
                                                 borigin4_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin4 +
                                                 borigin5_array_SRH_NT[movements$from[i],movements$to[i],iter]*origin5)/
          sum(exp(b0_array_SRH_NT[movements$from[i],possible_movements,iter] +
                    # btemp0_array_SRH_NT[movements$from[i],possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp + 
                    bspillwindow_array_SRH_NT[movements$from[i],possible_movements,iter]*spill_predict[j] +
                    # bwinterspill_array_SRH_NT[movements$from[i],possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRH_NT[movements$from[i],possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp0xorigin2_array_SRH_NT[movements$from[i],possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    # btemp0xorigin3_array_SRH_NT[movements$from[i],possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp0xorigin4_array_SRH_NT[movements$from[i],possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp0xorigin5_array_SRH_NT[movements$from[i],possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    borigin1_array_SRH_NT[movements$from[i],possible_movements,iter]*origin1 +
                    borigin2_array_SRH_NT[movements$from[i],possible_movements,iter]*origin2 +
                    borigin3_array_SRH_NT[movements$from[i],possible_movements,iter]*origin3 +
                    borigin4_array_SRH_NT[movements$from[i],possible_movements,iter]*origin4 +
                    borigin5_array_SRH_NT[movements$from[i],possible_movements,iter]*origin5))
...
TUC_wild_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Tucannon River", 
                                                                                      movements = TUC_spillvolume_movements)
TUC_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                            rear = "wild", origin_select = "Tucannon River")

ASO_wild_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Asotin Creek", 
                                                                                      movements = ASO_spillvolume_movements)
ASO_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                            rear = "wild", origin_select = "Asotin Creek")

CLE_wild_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Clearwater River", 
                                                                                      movements = CLE_spillvolume_movements)
CLE_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                            rear = "wild", origin_select = "Clearwater River")

SAL_wild_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Salmon River", 
                                                                                      movements = SAL_spillvolume_movements)
SAL_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                            rear = "wild", origin_select = "Salmon River")

GR_wild_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Grande Ronde River", 
                                                                                     movements = GR_spillvolume_movements)
GR_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                           rear = "wild", origin_select = "Grande Ronde River")

IMN_wild_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Imnaha River", 
                                                                                      movements = IMN_spillvolume_movements)
IMN_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                            rear = "wild", origin_select = "Imnaha River")

# Snake River, hatchery, transported
TUC_hatchery_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Tucannon River", 
                                                                                          movements = TUC_spillvolume_movements)
TUC_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                rear = "hatchery", origin_select = "Tucannon River")

CLE_hatchery_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Clearwater River", 
                                                                                          movements = CLE_spillvolume_movements)
CLE_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                rear = "hatchery", origin_select = "Clearwater River")

SAL_hatchery_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Salmon River", 
                                                                                          movements = SAL_spillvolume_movements)
SAL_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                rear = "hatchery", origin_select = "Salmon River")

GR_hatchery_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Grande Ronde River", 
                                                                                         movements = GR_spillvolume_movements)
GR_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                               rear = "hatchery", origin_select = "Grande Ronde River")

IMN_hatchery_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Imnaha River", 
                                                                                          movements = IMN_spillvolume_movements)
IMN_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                rear = "hatchery", origin_select = "Imnaha River")

# Snake River, wild, not_transported
TUC_wild_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Tucannon River", 
                                                                                          movements = TUC_spillvolume_movements)
TUC_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                                rear = "wild", origin_select = "Tucannon River")

ASO_wild_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Asotin Creek", 
                                                                                          movements = ASO_spillvolume_movements)
ASO_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                                rear = "wild", origin_select = "Asotin Creek")

CLE_wild_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Clearwater River", 
                                                                                          movements = CLE_spillvolume_movements)
CLE_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                                rear = "wild", origin_select = "Clearwater River")

SAL_wild_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Salmon River", 
                                                                                          movements = SAL_spillvolume_movements)
SAL_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                                rear = "wild", origin_select = "Salmon River")

GR_wild_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Grande Ronde River", 
                                                                                         movements = GR_spillvolume_movements)
GR_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                               rear = "wild", origin_select = "Grande Ronde River")

IMN_wild_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRW_T(origin_select = "Imnaha River", 
                                                                                          movements = IMN_spillvolume_movements)
IMN_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, 
                                                                                rear = "wild", origin_select = "Imnaha River")

# Snake River, hatchery, not_transported_
TUC_hatchery_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Tucannon River", 
                                                                                              movements = TUC_spillvolume_movements)
TUC_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                    rear = "hatchery", origin_select = "Tucannon River")

CLE_hatchery_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Clearwater River", 
                                                                                              movements = CLE_spillvolume_movements)
CLE_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                    rear = "hatchery", origin_select = "Clearwater River")

SAL_hatchery_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Salmon River", 
                                                                                              movements = SAL_spillvolume_movements)
SAL_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                    rear = "hatchery", origin_select = "Salmon River")

GR_hatchery_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Grande Ronde River", 
                                                                                             movements = GR_spillvolume_movements)
GR_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                   rear = "hatchery", origin_select = "Grande Ronde River")

IMN_hatchery_not_transported_spillvolume_move_prob_array <- estimate_spillwindow_effect_SRH_T(origin_select = "Imnaha River", 
                                                                                              movements = IMN_spillvolume_movements)
IMN_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, 
                                                                                    rear = "hatchery", origin_select = "Imnaha River")
...
reformat_spillwindow_prob_array <- function(move_prob_array, movements, origin_name, rear_type, transport_type){
  ...
  for (i in 1:dim(move_prob_array)[3]){
      ...
      movement_df %>% 
        ...
        mutate(origin = origin_name,
               rear = rear_type,
               juvenile_pathway = transport_type,
               from = movements$from[i],
...
### Snake River, wild, transported ###
TUC_natural_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = TUC_wild_transported_spillvolume_move_prob_array,
                                                                                 movements = TUC_spillvolume_movements,
                                                                                 origin_name = "Tucannon River",
                                                                                 transport_type = "transported", rear_type = "wild")

ASO_natural_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = ASO_wild_transported_spillvolume_move_prob_array,
                                                                                 movements = ASO_spillvolume_movements,
                                                                                 origin_name = "Asotin Creek",
                                                                                 transport_type = "transported", rear_type = "wild")

CLE_natural_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = CLE_wild_transported_spillvolume_move_prob_array,
                                                                                 movements = CLE_spillvolume_movements,
                                                                                 origin_name = "Clearwater River",
                                                                                 transport_type = "transported", rear_type = "wild")

SAL_natural_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = SAL_wild_transported_spillvolume_move_prob_array,
                                                                                 movements = SAL_spillvolume_movements,
                                                                                 origin_name = "Salmon River",
                                                                                 transport_type = "transported", rear_type = "wild")

GR_natural_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = GR_wild_transported_spillvolume_move_prob_array,
                                                                                movements = GR_spillvolume_movements,
                                                                                origin_name = "Grande Ronde River",
                                                                                transport_type = "transported", rear_type = "wild")

IMN_natural_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = IMN_wild_transported_spillvolume_move_prob_array,
                                                                                 movements = IMN_spillvolume_movements,
                                                                                 origin_name = "Imnaha River",
                                                                                 transport_type = "transported", rear_type = "wild")

### Snake River, hatchery, transported
TUC_hatchery_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = TUC_hatchery_transported_spillvolume_move_prob_array,
                                                                                  movements = TUC_spillvolume_movements,
                                                                                  origin_name = "Tucannon River",
                                                                                  transport_type = "transported", rear_type = "hatchery")

CLE_hatchery_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = CLE_hatchery_transported_spillvolume_move_prob_array,
                                                                                  movements = CLE_spillvolume_movements,
                                                                                  origin_name = "Clearwater River",
                                                                                  transport_type = "transported", rear_type = "hatchery")

SAL_hatchery_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = SAL_hatchery_transported_spillvolume_move_prob_array,
                                                                                  movements = SAL_spillvolume_movements,
                                                                                  origin_name = "Salmon River",
                                                                                  transport_type = "transported", rear_type = "hatchery")

GR_hatchery_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = GR_hatchery_transported_spillvolume_move_prob_array,
                                                                                 movements = GR_spillvolume_movements,
                                                                                 origin_name = "Grande Ronde River",
                                                                                 transport_type = "transported", rear_type = "hatchery")

IMN_hatchery_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = IMN_hatchery_transported_spillvolume_move_prob_array,
                                                                                  movements = IMN_spillvolume_movements,
                                                                                  origin_name = "Imnaha River",
                                                                                  transport_type = "transported", rear_type = "hatchery")

### Snake River, wild, not_transported ###
TUC_natural_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = TUC_wild_not_transported_spillvolume_move_prob_array,
                                                                                     movements = TUC_spillvolume_movements,
                                                                                     origin_name = "Tucannon River",
                                                                                     transport_type = "not_transported", rear_type = "wild")

ASO_natural_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = ASO_wild_not_transported_spillvolume_move_prob_array,
                                                                                     movements = ASO_spillvolume_movements,
                                                                                     origin_name = "Asotin Creek",
                                                                                     transport_type = "not_transported", rear_type = "wild")

CLE_natural_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = CLE_wild_not_transported_spillvolume_move_prob_array,
                                                                                     movements = CLE_spillvolume_movements,
                                                                                     origin_name = "Clearwater River",
                                                                                     transport_type = "not_transported", rear_type = "wild")

SAL_natural_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = SAL_wild_not_transported_spillvolume_move_prob_array,
                                                                                     movements = SAL_spillvolume_movements,
                                                                                     origin_name = "Salmon River",
                                                                                     transport_type = "not_transported", rear_type = "wild")

GR_natural_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = GR_wild_not_transported_spillvolume_move_prob_array,
                                                                                    movements = GR_spillvolume_movements,
                                                                                    origin_name = "Grande Ronde River",
                                                                                    transport_type = "not_transported", rear_type = "wild")

IMN_natural_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = IMN_wild_not_transported_spillvolume_move_prob_array,
                                                                                     movements = IMN_spillvolume_movements,
                                                                                     origin_name = "Imnaha River",
                                                                                     transport_type = "not_transported", rear_type = "wild")

### Snake River, hatchery, not_transported
TUC_hatchery_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = TUC_hatchery_not_transported_spillvolume_move_prob_array,
                                                                                      movements = TUC_spillvolume_movements,
                                                                                      origin_name = "Tucannon River",
                                                                                      transport_type = "not_transported", rear_type = "hatchery")

CLE_hatchery_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = CLE_hatchery_not_transported_spillvolume_move_prob_array,
                                                                                      movements = CLE_spillvolume_movements,
                                                                                      origin_name = "Clearwater River",
                                                                                      transport_type = "not_transported", rear_type = "hatchery")

SAL_hatchery_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = SAL_hatchery_not_transported_spillvolume_move_prob_array,
                                                                                      movements = SAL_spillvolume_movements,
                                                                                      origin_name = "Salmon River",
                                                                                      transport_type = "not_transported", rear_type = "hatchery")

GR_hatchery_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = GR_hatchery_not_transported_spillvolume_move_prob_array,
                                                                                     movements = GR_spillvolume_movements,
                                                                                     origin_name = "Grande Ronde River",
                                                                                     transport_type = "not_transported", rear_type = "hatchery")

IMN_hatchery_not_transported_spillvolume_move_long <- reformat_spillwindow_prob_array(move_prob_array = IMN_hatchery_not_transported_spillvolume_move_prob_array,
                                                                                      movements = IMN_spillvolume_movements,
                                                                                      origin_name = "Imnaha River",
                                                                                      transport_type = "not_transported", rear_type = "hatchery")
...
JDR_natural_spillvolume_move_long %>% 
  bind_rows( # MCW
    ...
    # SRW, T
    TUC_natural_transported_spillvolume_move_long, ASO_natural_transported_spillvolume_move_long,
    CLE_natural_transported_spillvolume_move_long, SAL_natural_transported_spillvolume_move_long,
    GR_natural_transported_spillvolume_move_long, IMN_natural_transported_spillvolume_move_long,
    # SRH, T
    TUC_hatchery_transported_spillvolume_move_long,
    CLE_hatchery_transported_spillvolume_move_long, SAL_hatchery_transported_spillvolume_move_long,
    GR_hatchery_transported_spillvolume_move_long, IMN_hatchery_transported_spillvolume_move_long, 
    # SRW, NT
    TUC_natural_not_transported_spillvolume_move_long, ASO_natural_not_transported_spillvolume_move_long,
    CLE_natural_not_transported_spillvolume_move_long, SAL_natural_not_transported_spillvolume_move_long,
    GR_natural_not_transported_spillvolume_move_long, IMN_natural_not_transported_spillvolume_move_long,
    # SRH, NT
    TUC_hatchery_not_transported_spillvolume_move_long,
    CLE_hatchery_not_transported_spillvolume_move_long, SAL_hatchery_not_transported_spillvolume_move_long,
    GR_hatchery_not_transported_spillvolume_move_long, IMN_hatchery_not_transported_spillvolume_move_long
    ) -> combined_spillvolume_move_long

combined_spillvolume_move_long %>% 
  group_by(spillwindow_actual, origin, rear, juvenile_pathway, from) %>% 
  summarise(prob = quantile(prob, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975)) %>% 
  pivot_wider(names_from = q, values_from = prob) %>% 
  mutate(origin_rear = paste0(origin, ", ", rear)) %>% 
  mutate(population = paste0(origin, ", ", rear, ", ", juvenile_pathway)) -> combined_spillvolume_move_summary
...
mutate(JDR_wild_covariate_experiences, population = "John Day River, natural, in-river",
       origin_rear = "John Day River, wild") %>% 
  bind_rows( # MCW
    mutate(UMA_wild_covariate_experiences, population = "Umatilla River, natural, in-river",
           origin_rear = "Umatila River, wild"),
    mutate(YAK_wild_covariate_experiences, population = "Yakima River, natural, in-river",
           origin_rear = "Yakima River, wild"), 
    mutate(WAWA_wild_covariate_experiences, population = "Walla Walla River, natural, in-river",
           origin_rear = "Walla Walla River, wild"), 
    mutate(FIF_wild_covariate_experiences, population = "Fifteenmile Creek, natural, in-river", origin_rear = "Fifteenmile Creek, wild"), 
    mutate(DES_wild_covariate_experiences, population = "Deschutes River, natural, in-river", origin_rear = "Deschutes River, wild"), 
    # MCH
    mutate(UMA_hatchery_covariate_experiences, population = "Umatilla River, hatchery, in-river", origin_rear = "Umatilla River, hatchery"), 
    mutate(WAWA_hatchery_covariate_experiences, population = "Walla Walla River, hatchery, in-river", origin_rear = "Walla Walla River, hatchery"), 
    # UCW
    mutate(WEN_wild_covariate_experiences, population = "Wenatchee River, natural, in-river", origin_rear = "Wenatchee River, wild"),  
    mutate(ENT_wild_covariate_experiences, population = "Entiat River, natural, in-river", origin_rear = "Entiat River, wild"), 
    mutate(MET_wild_covariate_experiences, population = "Methow River, natural, in-river", origin_rear = "Methow River, wild"), 
    # UCH
    mutate(WEN_hatchery_covariate_experiences, population = "Wenatchee River, hatchery, in-river", origin_rear = "Wenatchee River, hatchery"),  
    mutate(OKA_hatchery_covariate_experiences, population = "Okanogan River, hatchery, in-river", origin_rear = "Okanogan River, hatchery"), 
    mutate(MET_hatchery_covariate_experiences, population = "Methow River, hatchery, in-river", origin_rear = "Methow River, hatchery"), 
    # SRW, T
    mutate(TUC_wild_transported_covariate_experiences, population = "Tucannon River, natural, transported", origin_rear = "Tucannon River, wild"),  
    mutate(ASO_wild_transported_covariate_experiences, population = "Asotin Creek, natural, transported", origin_rear = "Asotin Creek, wild"), 
    mutate(CLE_wild_transported_covariate_experiences, population = "Clearwater River, natural, transported", origin_rear = "Clearwater River, wild"),  
    mutate(SAL_wild_transported_covariate_experiences, population = "Salmon River, natural, transported", origin_rear = "Salmon River, wild"), 
    mutate(GR_wild_transported_covariate_experiences, population = "Grande Ronde River, natural, transported", origin_rear = "Grande Ronde River, wild"),  
    mutate(IMN_wild_transported_covariate_experiences, population = "Imnaha River, natural, transported", origin_rear = "Imnaha River, wild"), 
    # SRH, T
    mutate(TUC_hatchery_transported_covariate_experiences, population = "Tucannon River, hatchery, in-river", origin_rear = "Tucannon River, hatchery"), 
    mutate(CLE_hatchery_transported_covariate_experiences, population = "Clearwater River, hatchery, in-river", origin_rear = "Clearwater River, hatchery"),  
    mutate(SAL_hatchery_transported_covariate_experiences, population = "Salmon River, hatchery, in-river", origin_rear = "Salmon River, hatchery"), 
    mutate(GR_hatchery_transported_covariate_experiences, population = "Grande Ronde River, hatchery, in-river", origin_rear = "Grande Ronde River, hatchery"),  
    mutate(IMN_hatchery_transported_covariate_experiences, population = "Imnaha River, hatchery, in-river", origin_rear = "Imnaha River, hatchery"), 
    # SRW, NT
    mutate(TUC_wild_not_transported_covariate_experiences, population = "Tucannon River, natural, transported", origin_rear = "Tucannon River, wild"),  
    mutate(ASO_wild_not_transported_covariate_experiences, population = "Asotin Creek, natural, transported", origin_rear = "Asotin Creek, wild"), 
    mutate(CLE_wild_not_transported_covariate_experiences, population = "Clearwater River, natural, transported", origin_rear = "Clearwater River, wild"),  
    mutate(SAL_wild_not_transported_covariate_experiences, population = "Salmon River, natural, transported", origin_rear = "Salmon River, wild"), 
    mutate(GR_wild_not_transported_covariate_experiences, population = "Grande Ronde River, natural, transported", origin_rear = "Grande Ronde River, wild"),  
    mutate(IMN_wild_not_transported_covariate_experiences, population = "Imnaha River, natural, transported", origin_rear = "Imnaha River, wild"), 
    # SRH, NT
    mutate(TUC_hatchery_not_transported_covariate_experiences, population = "Tucannon River, hatchery, in-river", origin_rear = "Tucannon River, hatchery"), 
    mutate(CLE_hatchery_not_transported_covariate_experiences, population = "Clearwater River, hatchery, in-river", origin_rear = "Clearwater River, hatchery"),  
    mutate(SAL_hatchery_not_transported_covariate_experiences, population = "Salmon River, hatchery, in-river", origin_rear = "Salmon River, hatchery"), 
    mutate(GR_hatchery_not_transported_covariate_experiences, population = "Grande Ronde River, hatchery, in-river", origin_rear = "Grande Ronde River, hatchery"),  
    mutate(IMN_hatchery_not_transported_covariate_experiences, population = "Imnaha River, hatchery, in-river", origin_rear = "Imnaha River, hatchery")
  ) %>% 
  mutate(spill_window_actual = spill_window * 100) %>% 
  mutate(juvenile_pathway = ifelse(grepl("in-river", population), "not_transported", "transported"))-> combined_covariate_experiences
...
plot_spill_fallback_onepop_onedam <- function(from_state, origin_rear_select = NA, DPS_select = NA, juvenile_pathway_select = NA,
                                              lower_spill_lim, upper_spill_lim){
  
  # can either subset a single origin/rear or a whole DPS
  if(!is.na(origin_rear_select)){
    if(!is.na(juvenile_pathway_select)){
      move_prob_data <- subset(combined_spillvolume_move_summary, from == from_state & origin_rear == origin_rear_select & juvenile_pathway == juvenile_pathway_select)
      covariate_data <- subset(combined_covariate_experiences, state == from_state & origin_rear == origin_rear_select & juvenile_pathway == juvenile_pathway_select)
    } else {
    ...
    if(!is.na(juvenile_pathway_select)){
      move_prob_data <- subset(combined_spillvolume_move_summary, from == from_state & DPS == DPS_select & juvenile_pathway == juvenile_pathway_select)
      covariate_data <- subset(combined_covariate_experiences, state == from_state & DPS == DPS_select & juvenile_pathway == juvenile_pathway_select)
    } else {
...
Snake_River_N_NT_BON_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 2, origin_rear_select = NA, 
                                                                        DPS_select = "Snake River, natural", juvenile_pathway_select = "not_transported",
                                                                        lower_spill_lim = 0, upper_spill_lim = 150)

Snake_River_H_NT_BON_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 2, origin_rear_select = NA, 
                                                                        DPS_select = "Snake River, hatchery", juvenile_pathway_select = "not_transported",
                                                                        lower_spill_lim = 0, upper_spill_lim = 150)

Snake_River_N_T_BON_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 2, origin_rear_select = NA, 
                                                                      DPS_select = "Snake River, natural", juvenile_pathway_select = "transported",
                                                                      lower_spill_lim = 0, upper_spill_lim = 150)

Snake_River_H_T_BON_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 2, origin_rear_select = NA, 
                                                                      DPS_select = "Snake River, hatchery", juvenile_pathway_select = "transported",
                                                                      lower_spill_lim = 0, upper_spill_lim = 150)
...
combined_BON_spill_fallback <- ggarrange(DES_N_BON_spill_fallback, FIF_N_BON_spill_fallback, JDR_N_BON_spill_fallback,
                                         UMA_N_BON_spill_fallback, UMA_H_BON_spill_fallback, WAWA_N_BON_spill_fallback, 
                                         WAWA_H_BON_spill_fallback, YAK_N_BON_spill_fallback, Snake_River_N_NT_BON_spill_fallback,
                                         Snake_River_N_T_BON_spill_fallback, Snake_River_H_NT_BON_spill_fallback,
                                         Snake_River_H_T_BON_spill_fallback, Upper_Columbia_N_BON_spill_fallback, Upper_Columbia_H_BON_spill_fallback, 
                                         nrow = 4, ncol = 4,
                                         labels = c("(A) DES, Natural", "(B) FIF, Natural",
                                                    ...
                                                    "(I) Snake R., Natural, in-river", "(J) Snake R., Natural, transported",
                                                    "(K) Snake R., Hatchery, in-river", "(L) Snake R., Hatchery, transported",
                                                    "(M) Upper Col., Natural", "(N) Upper Col., Hatchery"),
...
ggsave(here::here("figures", "transport", "paper_figures", "spill_volume", "BON_spill_fallback.png"), 
       combined_BON_spill_fallback, height = 10, width = 10)
...
TUC_N_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Tucannon River, wild", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

TUC_H_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Tucannon River, hatchery", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

ASO_N_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Asotin Creek, wild", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

CLE_N_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Clearwater River, wild", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

CLE_H_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Clearwater River, hatchery", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

IMN_N_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Imnaha River, wild", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

IMN_H_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Imnaha River, hatchery", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

GR_N_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Grande Ronde River, wild", 
                                                                DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

GR_H_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Grande Ronde River, hatchery", 
                                                                DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

SAL_N_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Salmon River, wild", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

SAL_H_NT_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Salmon River, hatchery", 
                                                                 DPS_select = NA, juvenile_pathway_select = "not_transported", lower_spill_lim = 0, upper_spill_lim = 150)

TUC_N_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Tucannon River, wild", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

TUC_H_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Tucannon River, hatchery", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

ASO_N_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Asotin Creek, wild", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

CLE_N_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Clearwater River, wild", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

CLE_H_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Clearwater River, hatchery", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

IMN_N_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Imnaha River, wild", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

IMN_H_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Imnaha River, hatchery", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

GR_N_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Grande Ronde River, wild", 
                                                               DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

GR_H_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Grande Ronde River, hatchery", 
                                                               DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

SAL_N_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Salmon River, wild", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)

SAL_H_T_MCN_spill_fallback <- plot_spill_fallback_onepop_onedam(from_state = 3, origin_rear_select = "Salmon River, hatchery", 
                                                                DPS_select = NA, juvenile_pathway_select = "transported", lower_spill_lim = 0, upper_spill_lim = 150)
...
combined_MCN_spill_fallback <- ggarrange(WAWA_N_MCN_spill_fallback,
                                         ...
                                         TUC_N_T_MCN_spill_fallback,
                                         TUC_N_NT_MCN_spill_fallback,
                                         TUC_H_T_MCN_spill_fallback,
                                         TUC_H_NT_MCN_spill_fallback,
                                         ASO_N_T_MCN_spill_fallback,
                                         ASO_N_NT_MCN_spill_fallback,
                                         CLE_N_T_MCN_spill_fallback,
                                         CLE_N_NT_MCN_spill_fallback,
                                         CLE_H_T_MCN_spill_fallback,
                                         CLE_H_NT_MCN_spill_fallback,
                                         IMN_N_T_MCN_spill_fallback,
                                         IMN_N_NT_MCN_spill_fallback,
                                         IMN_H_T_MCN_spill_fallback,
                                         IMN_H_NT_MCN_spill_fallback,
                                         GR_N_T_MCN_spill_fallback,
                                         GR_N_NT_MCN_spill_fallback,
                                         GR_H_T_MCN_spill_fallback,
                                         GR_H_NT_MCN_spill_fallback,
                                         SAL_N_T_MCN_spill_fallback,
                                         SAL_N_NT_MCN_spill_fallback,
                                         SAL_H_T_MCN_spill_fallback, 
                                         SAL_H_NT_MCN_spill_fallback, 
                                         nrow = 8, ncol = 4,
                                         labels = c("(A) WAWA, Natural", "(B) WAWA, Hatchery",
                                                    ...
                                                    "(I) MET, Hatchery", 
                                                    "(J) TUC, Natural, in-river",
                                                    "(K) TUC, Natural, transported",
                                                    "(L) TUC, Hatchery, in-river", 
                                                    "(M) TUC, Hatchery, transported", 
                                                    "(N) ASO, Natural, in-river",
                                                    "(O) ASO, Natural, transported",
                                                    "(P) CLE, Natural, in-river",
                                                    "(Q) CLE, Natural, transported",
                                                    "(R) CLE, Hatchery, in-river", 
                                                    "(S) CLE, Hatchery, transported", 
                                                    "(T) IMN, Natural, in-river",
                                                    "(U) IMN, Natural, transported",
                                                    "(V) IMN, Hatchery, in-river", 
                                                    "(W) IMN, Hatchery, transported", 
                                                    "(X) GR, Natural, in-river",
                                                    "(Y) GR, Natural, transported",
                                                    "(Z) GR, Hatchery, in-river", 
                                                    "(AA) GR, Hatchery, transported", 
                                                    "(AB) SAL, Natural, in-river",
                                                    "(AC) SAL, Natural, transported",
                                                    "(AD) SAL, Hatchery, in-river", 
                                                    "(AE) SAL, Hatchery, transported"),
                                         label.x = 0.05, label.y = 0.925, font.label = list(size = 10, face = "plain"),
                                         hjust = 0, vjust = 0)
...
ggsave(here::here("figures", "transport", "paper_figures", "spill_volume", "MCN_spill_fallback.png"), 
       combined_MCN_spill_fallback, height = 40*8/15, width = 10)
...
ggsave(here::here("figures", "transport", "paper_figures", "test_combined_density_plot.png"), combined_density_plot, height = 8, width = 10)
ggsave(here::here("figures", "transport", "paper_figures", "test_combined_spillvolume_plot.png"), combined_rear_spill_move_prob_plot, height = 8, width = 10)

combined_combined_plot <- ggarrange(combined_density_plot, combined_rear_spill_move_prob_plot, nrow = 2, ncol = 1,
                           heights = c(2,6))

ggsave(here::here("figures", "transport", "paper_figures", "test_spill_combined_combined_plot.png"), 
...
ggsave(here::here("figures", "transport", "paper_figures", "MCN_test_spill_combined_combined_plot.png"), 
...
ggsave(here::here("figures", "transport", "paper_figures", "ICH_test_spill_combined_combined_plot.png"), 
...
ggsave(here::here("figures", "transport", "paper_figures", "LGR_test_spill_combined_combined_plot.png"), 
