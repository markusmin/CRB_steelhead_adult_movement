...
estimate_spilldays_effect_SRW_T <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    SRW_T_states_dates <- data.frame(state = as.vector(SRW_T_envir$data$y),
                                     date_numeric = as.vector(SRW_T_envir$data$transition_dates))
    
    # add month to states dates so that we can subset
    SRW_T_states_dates %>% 
      mutate(date = ymd("2005-05-31") + days(date_numeric)) %>% 
      mutate(month = month(date)) -> SRW_T_states_dates
    
    # get median temperature for this state - only in Jan/Feb/Mar
    temp_data <- as.data.frame(SRW_T_envir$data$temperature)
    temp_data %>% 
      rownames_to_column("date_numeric") %>% 
      pivot_longer(cols = -c(date_numeric)) -> temp_data_long
    ...
    # only keep actual observed fish
    med_temp <- median(temp_data[subset(SRW_T_states_dates, state == from & month %in% c(1,2,3))$date_numeric,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_days_predict)){
        ...
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRW_T[movements$from[i],movements$to[i],iter] +
                                                 btemp0_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # btemp1_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # bspillwindow_array_SRW_T[movements$from[i],movements$to[i],iter]*med_spillwindow + # no spill window if spill days is indexed
                                                 bwinterspill_array_SRW_T[movements$from[i],movements$to[i],iter]*spill_days_predict[j] +
                                                 btemp0xorigin1_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp1xorigin1_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 btemp0xorigin2_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin2 +
                                                 # btemp1xorigin2_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 btemp0xorigin3_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp1xorigin3_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 btemp0xorigin4_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp1xorigin4_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 btemp0xorigin5_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 # btemp1xorigin5_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 btemp0xorigin6_array_SRW_T[movements$from[i],movements$to[i],iter]*med_temp*origin6 +
          ...
          sum(exp(b0_array_SRW_T[movements$from[i],possible_movements,iter] +
                    btemp0_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp + # select temp0 for any winterspill estimates
                    # btemp1_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp + 
                    # bspillwindow_array_SRW_T[movements$from[i],possible_movements,iter]*med_spillwindow + # no spill window if spill days is indexed
                    bwinterspill_array_SRW_T[movements$from[i],possible_movements,iter]*spill_days_predict[j] +
                    btemp0xorigin1_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp1xorigin1_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    btemp0xorigin2_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin2 +
                    # btemp1xorigin2_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    btemp0xorigin3_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp1xorigin3_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    btemp0xorigin4_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp1xorigin4_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    btemp0xorigin5_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    # btemp1xorigin5_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    btemp0xorigin6_array_SRW_T[movements$from[i],possible_movements,iter]*med_temp*origin6 +
...
estimate_spilldays_effect_SRW_NT <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    SRW_NT_states_dates <- data.frame(state = as.vector(SRW_NT_envir$data$y),
                                      date_numeric = as.vector(SRW_NT_envir$data$transition_dates))
    
    # add month to states dates so that we can subset
    SRW_NT_states_dates %>% 
      mutate(date = ymd("2005-05-31") + days(date_numeric)) %>% 
      mutate(month = month(date)) -> SRW_NT_states_dates
    
    # get median temperature for this state - only in Jan/Feb/Mar
    temp_data <- as.data.frame(SRW_NT_envir$data$temperature)
    temp_data %>% 
      rownames_to_column("date_numeric") %>% 
      pivot_longer(cols = -c(date_numeric)) -> temp_data_long
    ...
    # only keep actual observed fish
    med_temp <- median(temp_data[subset(SRW_NT_states_dates, state == from & month %in% c(1,2,3))$date_numeric,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_days_predict)){
        ...
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRW_NT[movements$from[i],movements$to[i],iter] +
                                                 btemp0_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # btemp1_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # bspillwindow_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_spillwindow + # no spill window if spill days is indexed
                                                 bwinterspill_array_SRW_NT[movements$from[i],movements$to[i],iter]*spill_days_predict[j] +
                                                 btemp0xorigin1_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp1xorigin1_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 btemp0xorigin2_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin2 +
                                                 # btemp1xorigin2_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 btemp0xorigin3_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp1xorigin3_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 btemp0xorigin4_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp1xorigin4_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 btemp0xorigin5_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 # btemp1xorigin5_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
                                                 btemp0xorigin6_array_SRW_NT[movements$from[i],movements$to[i],iter]*med_temp*origin6 +
          ...
          sum(exp(b0_array_SRW_NT[movements$from[i],possible_movements,iter] +
                    btemp0_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp + # select temp0 for any winterspill estimates
                    # btemp1_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp + 
                    # bspillwindow_array_SRW_NT[movements$from[i],possible_movements,iter]*med_spillwindow + # no spill window if spill days is indexed
                    bwinterspill_array_SRW_NT[movements$from[i],possible_movements,iter]*spill_days_predict[j] +
                    btemp0xorigin1_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp1xorigin1_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    btemp0xorigin2_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin2 +
                    # btemp1xorigin2_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    btemp0xorigin3_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp1xorigin3_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    btemp0xorigin4_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp1xorigin4_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    btemp0xorigin5_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    # btemp1xorigin5_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin5 +
                    btemp0xorigin6_array_SRW_NT[movements$from[i],possible_movements,iter]*med_temp*origin6 +
...
estimate_spilldays_effect_SRH_T <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    SRH_T_states_dates <- data.frame(state = as.vector(SRH_T_envir$data$y),
                                     date_numeric = as.vector(SRH_T_envir$data$transition_dates))
    
    # add month to states dates so that we can subset
    SRH_T_states_dates %>% 
      mutate(date = ymd("2005-05-31") + days(date_numeric)) %>% 
      mutate(month = month(date)) -> SRH_T_states_dates
    
    # get median temperature for this state - only in Jan/Feb/Mar
    temp_data <- as.data.frame(SRH_T_envir$data$temperature)
    temp_data %>% 
      rownames_to_column("date_numeric") %>% 
      pivot_longer(cols = -c(date_numeric)) -> temp_data_long
    ...
    # only keep actual observed fish
    med_temp <- median(temp_data[subset(SRH_T_states_dates, state == from & month %in% c(1,2,3))$date_numeric,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_days_predict)){
        ...
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRH_T[movements$from[i],movements$to[i],iter] +
                                                 btemp0_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # btemp1_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # bspillwindow_array_SRH_T[movements$from[i],movements$to[i],iter]*med_spillwindow + # no spill window if spill days is indexed
                                                 bwinterspill_array_SRH_T[movements$from[i],movements$to[i],iter]*spill_days_predict[j] +
                                                 btemp0xorigin1_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp1xorigin1_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 btemp0xorigin2_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin2 +
                                                 # btemp1xorigin2_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 btemp0xorigin3_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp1xorigin3_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 btemp0xorigin4_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp1xorigin4_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 btemp0xorigin5_array_SRH_T[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
          ...
          sum(exp(b0_array_SRH_T[movements$from[i],possible_movements,iter] +
                    btemp0_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp + # select temp0 for any winterspill estimates
                    # btemp1_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp + 
                    # bspillwindow_array_SRH_T[movements$from[i],possible_movements,iter]*med_spillwindow + # no spill window if spill days is indexed
                    bwinterspill_array_SRH_T[movements$from[i],possible_movements,iter]*spill_days_predict[j] +
                    btemp0xorigin1_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp1xorigin1_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    btemp0xorigin2_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin2 +
                    # btemp1xorigin2_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    btemp0xorigin3_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp1xorigin3_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    btemp0xorigin4_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp1xorigin4_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    btemp0xorigin5_array_SRH_T[movements$from[i],possible_movements,iter]*med_temp*origin5 +
...
estimate_spilldays_effect_SRH_NT <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    SRH_NT_states_dates <- data.frame(state = as.vector(SRH_NT_envir$data$y),
                                      date_numeric = as.vector(SRH_NT_envir$data$transition_dates))
    
    # add month to states dates so that we can subset
    SRH_NT_states_dates %>% 
      mutate(date = ymd("2005-05-31") + days(date_numeric)) %>% 
      mutate(month = month(date)) -> SRH_NT_states_dates
    
    # get median temperature for this state - only in Jan/Feb/Mar
    temp_data <- as.data.frame(SRH_NT_envir$data$temperature)
    temp_data %>% 
      rownames_to_column("date_numeric") %>% 
      pivot_longer(cols = -c(date_numeric)) -> temp_data_long
    ...
    # only keep actual observed fish
    med_temp <- median(temp_data[subset(SRH_NT_states_dates, state == from & month %in% c(1,2,3))$date_numeric,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(spill_days_predict)){
        ...
        spill_move_prob_array[j,iter,i] <- exp(b0_array_SRH_NT[movements$from[i],movements$to[i],iter] +
                                                 btemp0_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # btemp1_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp + # select temp0 for any winterspill estimates
                                                 # bspillwindow_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_spillwindow + # no spill window if spill days is indexed
                                                 bwinterspill_array_SRH_NT[movements$from[i],movements$to[i],iter]*spill_days_predict[j] +
                                                 btemp0xorigin1_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 # btemp1xorigin1_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin1 +
                                                 btemp0xorigin2_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin2 +
                                                 # btemp1xorigin2_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin2 + 
                                                 btemp0xorigin3_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 # btemp1xorigin3_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin3 +
                                                 btemp0xorigin4_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 # btemp1xorigin4_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin4 +
                                                 btemp0xorigin5_array_SRH_NT[movements$from[i],movements$to[i],iter]*med_temp*origin5 +
          ...
          sum(exp(b0_array_SRH_NT[movements$from[i],possible_movements,iter] +
                    btemp0_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp + # select temp0 for any winterspill estimates
                    # btemp1_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp + 
                    # bspillwindow_array_SRH_NT[movements$from[i],possible_movements,iter]*med_spillwindow + # no spill window if spill days is indexed
                    bwinterspill_array_SRH_NT[movements$from[i],possible_movements,iter]*spill_days_predict[j] +
                    btemp0xorigin1_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    # btemp1xorigin1_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin1 +
                    btemp0xorigin2_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin2 +
                    # btemp1xorigin2_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin2 + 
                    btemp0xorigin3_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    # btemp1xorigin3_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin3 +
                    btemp0xorigin4_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    # btemp1xorigin4_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin4 +
                    btemp0xorigin5_array_SRH_NT[movements$from[i],possible_movements,iter]*med_temp*origin5 +
...
plot_compare_rear_spill_effect_multiple_movements <- function(origin_select,
  ...
  if (plot_legend == TRUE){
    ...
    # for testing
    # ggsave(here::here("figures", "transport", "paper_figures",  "01_legend_test.png"), spill_plot_legend_gg, height = 4, width = 4)
...
TUC_spilldays_movements <- data.frame(from = c(9,9), to = c(8, 41),
                                      Movement = c("Fallback",
                                                   "Loss"))
TUC_wild_transported_spill_move_prob_array <- estimate_spilldays_effect_SRW_T(origin_select = "Tucannon River", movements = TUC_spilldays_movements)
TUC_wild_not_transported_spill_move_prob_array <- estimate_spilldays_effect_SRW_NT(origin_select = "Tucannon River", movements = TUC_spilldays_movements)
TUC_hatchery_transported_spill_move_prob_array <- estimate_spilldays_effect_SRH_T(origin_select = "Tucannon River", movements = TUC_spilldays_movements)
TUC_hatchery_not_transported_spill_move_prob_array <- estimate_spilldays_effect_SRH_NT(origin_select = "Tucannon River", movements = TUC_spilldays_movements)
...
ggsave(here::here("figures", "transport", "spilldays", "JDR_wild_compare_movement_spill.png"), JDR_wild_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "UMA_wild_compare_movement_spill.png"), UMA_wild_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "UMA_hatchery_compare_movement_spill.png"), UMA_hatchery_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "YAK_wild_compare_movement_spill.png"), YAK_wild_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "WAWA_wild_compare_movement_spill.png"), WAWA_wild_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "WAWA_hatchery_compare_movement_spill.png"), WAWA_hatchery_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "WEN_wild_compare_movement_spill.png"), WEN_wild_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "WEN_hatchery_compare_movement_spill.png"), WEN_hatchery_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "ENT_wild_compare_movement_spill.png"), ENT_wild_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "TUC_wild_transported_compare_movement_spill.png"), TUC_wild_transported_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "TUC_wild_not_transported_compare_movement_spill.png"), TUC_wild_not_transported_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "TUC_hatchery_transported_compare_movement_spill.png"), TUC_hatchery_transported_compare_movement_spill, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "TUC_hatchery_not_transported_compare_movement_spill.png"), TUC_hatchery_not_transported_compare_movement_spill, height = 8, width = 8)
...
# 
# ggsave(here::here("figures", "transport", "spilldays", "IMN_wild_compare_movement_spill.png"), IMN_wild_compare_movement_spill, height = 8, width = 8)
# 
# # IMN hatchery plot
...
#                                                                                          movements_evaluated = IMN_spilldays_movements)
# 
# ggsave(here::here("figures", "transport", "spilldays", "IMN_hatchery_compare_movement_spill.png"), IMN_hatchery_compare_movement_spill, height = 8, width = 8)
...
#                                           hjust = 0, vjust = 0)
# 
# ggsave(here::here("figures", "transport", "paper_figures",  "combined_movement_spill_plot_v1.png"), combined_movement_spill_plot, height = 12, width = 16)
...
ggsave(here::here("figures", "transport", "paper_figures",  "Fig6_spilldays_plot.png"), combined_movement_spill_plot, height = 12, width = 16)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "MCN_spill_fallback_plot.png"), MCN_spill_fallback_plot, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "PRA_spill_fallback_plot.png"), PRA_spill_fallback_plot, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "RIS_spill_fallback_plot.png"), RIS_spill_fallback_plot, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "RRE_spill_fallback_plot.png"), RRE_spill_fallback_plot, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "WEL_spill_fallback_plot.png"), WEL_spill_fallback_plot, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "ICH_spill_fallback_plot.png"), ICH_spill_fallback_plot, height = 8, width = 8)
...
TUC_wild_not_transported_fallback_LGR_spill_move_prob_array <- estimate_spilldays_effect_SRW_NT(origin_select = "Tucannon River", movements = LGR_spilldays_movements)
TUC_wild_transported_fallback_LGR_spill_move_prob_array <- estimate_spilldays_effect_SRW_T(origin_select = "Tucannon River", movements = LGR_spilldays_movements)
TUC_hatchery_not_transported_fallback_LGR_spill_move_prob_array <- estimate_spilldays_effect_SRH_NT(origin_select = "Tucannon River", movements = LGR_spilldays_movements)
TUC_hatchery_transported_fallback_LGR_spill_move_prob_array <- estimate_spilldays_effect_SRH_T(origin_select = "Tucannon River", movements = LGR_spilldays_movements)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet", "LGR_spill_fallback_plot.png"), LGR_spill_fallback_plot, height = 8, width = 8)
...
ggsave(here::here("figures", "transport", "spilldays", "dam_facet",  "dam_facet_spilldays_plot.png"), combined_dam_spill_plot, height = 10, width = 10)
