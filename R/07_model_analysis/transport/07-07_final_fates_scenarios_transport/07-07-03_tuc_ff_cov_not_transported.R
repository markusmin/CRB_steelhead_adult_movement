...
source("R/07_model_analysis/transport/07-07_final_fates_scenarios_transport/07-07-01-ff_cov_functions_transport.R")
...
for (i in 1:length(LGR_winter_spill_values)){
  for (j in 1:length(fix_run_years)){
    TUC_FF <- compare_final_fate_fixcov_rear_type_SR_NT(niter = ff_iter, nsim = ff_nsim,
...
# save output
save(TUC_LGR_winterspill_homing, file = "figures/transport/final_fates_scenarios/simulation_runs/TUC_LGR_not_transported_homing.rda")