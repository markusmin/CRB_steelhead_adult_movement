...
SRW_T_DE_param_matrix <- extract_DE_parameters(fit = SRW_T_fit, fit_summary = SRW_T_fit_summary)
SRW_NT_DE_param_matrix <- extract_DE_parameters(fit = SRW_NT_fit, fit_summary = SRW_NT_fit_summary)
SRH_T_DE_param_matrix <- extract_DE_parameters(fit = SRH_T_fit, fit_summary = SRH_T_fit_summary)
SRH_NT_DE_param_matrix <- extract_DE_parameters(fit = SRH_NT_fit, fit_summary = SRH_NT_fit_summary)
...
estimate_temp_effect_SRW_T <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    if (from %in% c(1:9)){
      ...
      med_spillwindow <- median(spillwindow_data[subset(SRW_T_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(temp_predict)){
        # evaluation movement 
        temp_move_prob_array[j,iter,i] <- exp(b0_array_SRW_T[from,to,iter] +
                                                # btemp0_array_SRW_T[from,to,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                btemp1_array_SRW_T[from,to,iter]*temp_predict[j] + 
                                                bspillwindow_array_SRW_T[from,to,iter]*med_spillwindow +
                                                bwinterspill_array_SRW_T[from,to,iter]*med_winterspill +
                                                # btemp0xorigin1_array_SRW_T[from,to,iter]*origin1 +
                                                btemp1xorigin1_array_SRW_T[from,to,iter]*temp_predict[j]*origin1 +
                                                # btemp0xorigin2_array_SRW_T[from,to,iter]*origin2 + 
                                                btemp1xorigin2_array_SRW_T[from,to,iter]*temp_predict[j]*origin2 + 
                                                # btemp0xorigin3_array_SRW_T[from,to,iter]*origin3 +
                                                btemp1xorigin3_array_SRW_T[from,to,iter]*temp_predict[j]*origin3 +
                                                # btemp0xorigin4_array_SRW_T[from,to,iter]*origin4 +
                                                btemp1xorigin4_array_SRW_T[from,to,iter]*temp_predict[j]*origin4 +
                                                # btemp0xorigin5_array_SRW_T[from,to,iter]*origin5 +
                                                btemp1xorigin5_array_SRW_T[from,to,iter]*temp_predict[j]*origin5 +
                                                # btemp0xorigin6_array_SRW_T[from,to,iter]*origin6 +
                                                btemp1xorigin6_array_SRW_T[from,to,iter]*temp_predict[j]*origin6 +
                                                borigin1_array_SRW_T[from,to,iter]*origin1 +
                                                borigin2_array_SRW_T[from,to,iter]*origin2 +
                                                borigin3_array_SRW_T[from,to,iter]*origin3 +
                                                borigin4_array_SRW_T[from,to,iter]*origin4 +
                                                borigin5_array_SRW_T[from,to,iter]*origin5 +
                                                borigin6_array_SRW_T[from,to,iter]*origin6)/
          sum(exp(b0_array_SRW_T[from,possible_movements,iter] +
                    # btemp0_array_SRW_T[from,possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRW_T[from,possible_movements,iter]*temp_predict[j] + 
                    bspillwindow_array_SRW_T[from,possible_movements,iter]*med_spillwindow +
                    bwinterspill_array_SRW_T[from,possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRW_T[from,possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRW_T[from,possible_movements,iter]*temp_predict[j]*origin1 +
                    # btemp0xorigin2_array_SRW_T[from,possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRW_T[from,possible_movements,iter]*temp_predict[j]*origin2 + 
                    # btemp0xorigin3_array_SRW_T[from,possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRW_T[from,possible_movements,iter]*temp_predict[j]*origin3 +
                    # btemp0xorigin4_array_SRW_T[from,possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRW_T[from,possible_movements,iter]*temp_predict[j]*origin4 +
                    # btemp0xorigin5_array_SRW_T[from,possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRW_T[from,possible_movements,iter]*temp_predict[j]*origin5 +
                    # btemp0xorigin6_array_SRW_T[from,possible_movements,iter]*origin6 +
                    btemp1xorigin6_array_SRW_T[from,possible_movements,iter]*temp_predict[j]*origin6 +
                    borigin1_array_SRW_T[from,possible_movements,iter]*origin1 +
                    borigin2_array_SRW_T[from,possible_movements,iter]*origin2 +
                    borigin3_array_SRW_T[from,possible_movements,iter]*origin3 +
                    borigin4_array_SRW_T[from,possible_movements,iter]*origin4 +
                    borigin5_array_SRW_T[from,possible_movements,iter]*origin5 +
                    borigin6_array_SRW_T[from,possible_movements,iter]*origin6))
...
estimate_temp_effect_SRW_NT <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    if (from %in% c(1:9)){
      ...
      med_spillwindow <- median(spillwindow_data[subset(SRW_NT_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(temp_predict)){
        # evaluation movement 
        temp_move_prob_array[j,iter,i] <- exp(b0_array_SRW_NT[from,to,iter] +
                                                # btemp0_array_SRW_NT[from,to,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                btemp1_array_SRW_NT[from,to,iter]*temp_predict[j] + 
                                                bspillwindow_array_SRW_NT[from,to,iter]*med_spillwindow +
                                                bwinterspill_array_SRW_NT[from,to,iter]*med_winterspill +
                                                # btemp0xorigin1_array_SRW_NT[from,to,iter]*origin1 +
                                                btemp1xorigin1_array_SRW_NT[from,to,iter]*temp_predict[j]*origin1 +
                                                # btemp0xorigin2_array_SRW_NT[from,to,iter]*origin2 + 
                                                btemp1xorigin2_array_SRW_NT[from,to,iter]*temp_predict[j]*origin2 + 
                                                # btemp0xorigin3_array_SRW_NT[from,to,iter]*origin3 +
                                                btemp1xorigin3_array_SRW_NT[from,to,iter]*temp_predict[j]*origin3 +
                                                # btemp0xorigin4_array_SRW_NT[from,to,iter]*origin4 +
                                                btemp1xorigin4_array_SRW_NT[from,to,iter]*temp_predict[j]*origin4 +
                                                # btemp0xorigin5_array_SRW_NT[from,to,iter]*origin5 +
                                                btemp1xorigin5_array_SRW_NT[from,to,iter]*temp_predict[j]*origin5 +
                                                # btemp0xorigin6_array_SRW_NT[from,to,iter]*origin6 +
                                                btemp1xorigin6_array_SRW_NT[from,to,iter]*temp_predict[j]*origin6 +
                                                borigin1_array_SRW_NT[from,to,iter]*origin1 +
                                                borigin2_array_SRW_NT[from,to,iter]*origin2 +
                                                borigin3_array_SRW_NT[from,to,iter]*origin3 +
                                                borigin4_array_SRW_NT[from,to,iter]*origin4 +
                                                borigin5_array_SRW_NT[from,to,iter]*origin5 +
                                                borigin6_array_SRW_NT[from,to,iter]*origin6)/
          sum(exp(b0_array_SRW_NT[from,possible_movements,iter] +
                    # btemp0_array_SRW_NT[from,possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRW_NT[from,possible_movements,iter]*temp_predict[j] + 
                    bspillwindow_array_SRW_NT[from,possible_movements,iter]*med_spillwindow +
                    bwinterspill_array_SRW_NT[from,possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRW_NT[from,possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRW_NT[from,possible_movements,iter]*temp_predict[j]*origin1 +
                    # btemp0xorigin2_array_SRW_NT[from,possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRW_NT[from,possible_movements,iter]*temp_predict[j]*origin2 + 
                    # btemp0xorigin3_array_SRW_NT[from,possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRW_NT[from,possible_movements,iter]*temp_predict[j]*origin3 +
                    # btemp0xorigin4_array_SRW_NT[from,possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRW_NT[from,possible_movements,iter]*temp_predict[j]*origin4 +
                    # btemp0xorigin5_array_SRW_NT[from,possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRW_NT[from,possible_movements,iter]*temp_predict[j]*origin5 +
                    # btemp0xorigin6_array_SRW_NT[from,possible_movements,iter]*origin6 +
                    btemp1xorigin6_array_SRW_NT[from,possible_movements,iter]*temp_predict[j]*origin6 +
                    borigin1_array_SRW_NT[from,possible_movements,iter]*origin1 +
                    borigin2_array_SRW_NT[from,possible_movements,iter]*origin2 +
                    borigin3_array_SRW_NT[from,possible_movements,iter]*origin3 +
                    borigin4_array_SRW_NT[from,possible_movements,iter]*origin4 +
                    borigin5_array_SRW_NT[from,possible_movements,iter]*origin5 +
                    borigin6_array_SRW_NT[from,possible_movements,iter]*origin6))
...
estimate_temp_effect_SRH_T <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    if (from %in% c(1:9)){
      ...
      med_spillwindow <- median(spillwindow_data[subset(SRH_T_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(temp_predict)){
        # evaluation movement 
        temp_move_prob_array[j, iter,i] <- exp(b0_array_SRH_T[from,to,iter] +
                                                 # btemp0_array_SRH_T[from,to,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                 btemp1_array_SRH_T[from,to,iter]*temp_predict[j] + 
                                                 bspillwindow_array_SRH_T[from,to,iter]*med_spillwindow +
                                                 bwinterspill_array_SRH_T[from,to,iter]*med_winterspill +
                                                 # btemp0xorigin1_array_SRH_T[from,to,iter]*origin1 +
                                                 btemp1xorigin1_array_SRH_T[from,to,iter]*temp_predict[j]*origin1 +
                                                 # btemp0xorigin2_array_SRH_T[from,to,iter]*origin2 + 
                                                 btemp1xorigin2_array_SRH_T[from,to,iter]*temp_predict[j]*origin2 + 
                                                 # btemp0xorigin3_array_SRH_T[from,to,iter]*origin3 +
                                                 btemp1xorigin3_array_SRH_T[from,to,iter]*temp_predict[j]*origin3 +
                                                 # btemp0xorigin4_array_SRH_T[from,to,iter]*origin4 +
                                                 btemp1xorigin4_array_SRH_T[from,to,iter]*temp_predict[j]*origin4 +
                                                 # btemp0xorigin5_array_SRH_T[from,to,iter]*origin5 +
                                                 btemp1xorigin5_array_SRH_T[from,to,iter]*temp_predict[j]*origin5 +
                                                 borigin1_array_SRH_T[from,to,iter]*origin1 +
                                                 borigin2_array_SRH_T[from,to,iter]*origin2 +
                                                 borigin3_array_SRH_T[from,to,iter]*origin3 +
                                                 borigin4_array_SRH_T[from,to,iter]*origin4 +
                                                 borigin5_array_SRH_T[from,to,iter]*origin5)/
          sum(exp(b0_array_SRH_T[from,possible_movements,iter] +
                    # btemp0_array_SRH_T[from,possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRH_T[from,possible_movements,iter]*temp_predict[j] + 
                    bspillwindow_array_SRH_T[from,possible_movements,iter]*med_spillwindow +
                    bwinterspill_array_SRH_T[from,possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRH_T[from,possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRH_T[from,possible_movements,iter]*temp_predict[j]*origin1 +
                    # btemp0xorigin2_array_SRH_T[from,possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRH_T[from,possible_movements,iter]*temp_predict[j]*origin2 + 
                    # btemp0xorigin3_array_SRH_T[from,possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRH_T[from,possible_movements,iter]*temp_predict[j]*origin3 +
                    # btemp0xorigin4_array_SRH_T[from,possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRH_T[from,possible_movements,iter]*temp_predict[j]*origin4 +
                    # btemp0xorigin5_array_SRH_T[from,possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRH_T[from,possible_movements,iter]*temp_predict[j]*origin5 +
                    borigin1_array_SRH_T[from,possible_movements,iter]*origin1 +
                    borigin2_array_SRH_T[from,possible_movements,iter]*origin2 +
                    borigin3_array_SRH_T[from,possible_movements,iter]*origin3 +
                    borigin4_array_SRH_T[from,possible_movements,iter]*origin4 +
                    borigin5_array_SRH_T[from,possible_movements,iter]*origin5))
...
estimate_temp_effect_SRH_NT <- function(origin_select, movements){
  ...
  for (i in 1:nrow(movements)){
    ...
    if (from %in% c(1:9)){
      ...
      med_spillwindow <- median(spillwindow_data[subset(SRH_NT_states_dates, state == from)$date,from])
    ...
    for (iter in 1:niter){
      ...
      for (j in 1:length(temp_predict)){
        # evaluation movement 
        temp_move_prob_array[j, iter,i] <- exp(b0_array_SRH_NT[from,to,iter] +
                                                 # btemp0_array_SRH_NT[from,to,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                                                 btemp1_array_SRH_NT[from,to,iter]*temp_predict[j] + 
                                                 bspillwindow_array_SRH_NT[from,to,iter]*med_spillwindow +
                                                 bwinterspill_array_SRH_NT[from,to,iter]*med_winterspill +
                                                 # btemp0xorigin1_array_SRH_NT[from,to,iter]*origin1 +
                                                 btemp1xorigin1_array_SRH_NT[from,to,iter]*temp_predict[j]*origin1 +
                                                 # btemp0xorigin2_array_SRH_NT[from,to,iter]*origin2 + 
                                                 btemp1xorigin2_array_SRH_NT[from,to,iter]*temp_predict[j]*origin2 + 
                                                 # btemp0xorigin3_array_SRH_NT[from,to,iter]*origin3 +
                                                 btemp1xorigin3_array_SRH_NT[from,to,iter]*temp_predict[j]*origin3 +
                                                 # btemp0xorigin4_array_SRH_NT[from,to,iter]*origin4 +
                                                 btemp1xorigin4_array_SRH_NT[from,to,iter]*temp_predict[j]*origin4 +
                                                 # btemp0xorigin5_array_SRH_NT[from,to,iter]*origin5 +
                                                 btemp1xorigin5_array_SRH_NT[from,to,iter]*temp_predict[j]*origin5 +
                                                 borigin1_array_SRH_NT[from,to,iter]*origin1 +
                                                 borigin2_array_SRH_NT[from,to,iter]*origin2 +
                                                 borigin3_array_SRH_NT[from,to,iter]*origin3 +
                                                 borigin4_array_SRH_NT[from,to,iter]*origin4 +
                                                 borigin5_array_SRH_NT[from,to,iter]*origin5)/
          sum(exp(b0_array_SRH_NT[from,possible_movements,iter] +
                    # btemp0_array_SRH_NT[from,possible_movements,iter] + # ignore this parameter - because the vast majority of transitions occur in summer/fall, which corresponds to temp1 param
                    btemp1_array_SRH_NT[from,possible_movements,iter]*temp_predict[j] + 
                    bspillwindow_array_SRH_NT[from,possible_movements,iter]*med_spillwindow +
                    bwinterspill_array_SRH_NT[from,possible_movements,iter]*med_winterspill +
                    # btemp0xorigin1_array_SRH_NT[from,possible_movements,iter]*origin1 +
                    btemp1xorigin1_array_SRH_NT[from,possible_movements,iter]*temp_predict[j]*origin1 +
                    # btemp0xorigin2_array_SRH_NT[from,possible_movements,iter]*origin2 + 
                    btemp1xorigin2_array_SRH_NT[from,possible_movements,iter]*temp_predict[j]*origin2 + 
                    # btemp0xorigin3_array_SRH_NT[from,possible_movements,iter]*origin3 +
                    btemp1xorigin3_array_SRH_NT[from,possible_movements,iter]*temp_predict[j]*origin3 +
                    # btemp0xorigin4_array_SRH_NT[from,possible_movements,iter]*origin4 +
                    btemp1xorigin4_array_SRH_NT[from,possible_movements,iter]*temp_predict[j]*origin4 +
                    # btemp0xorigin5_array_SRH_NT[from,possible_movements,iter]*origin5 +
                    btemp1xorigin5_array_SRH_NT[from,possible_movements,iter]*temp_predict[j]*origin5 +
                    borigin1_array_SRH_NT[from,possible_movements,iter]*origin1 +
                    borigin2_array_SRH_NT[from,possible_movements,iter]*origin2 +
                    borigin3_array_SRH_NT[from,possible_movements,iter]*origin3 +
                    borigin4_array_SRH_NT[from,possible_movements,iter]*origin4 +
                    borigin5_array_SRH_NT[from,possible_movements,iter]*origin5))
...
plot_compare_rear_temp_effect_multiple_movements <- function(origin_select,
  ...
  if (plot_legend == TRUE){
    ...
    # for testing
    # ggsave(here::here("figures", "transport","paper_figures", "01_legend_test.png"), temp_plot_legend_gg, height = 4, width = 4)
    ...
    # for testing
    # ggsave(here::here("figures", "transport","paper_figures", "01_test.png"), combined_plot, height = 6, width = 6)
...
TUC_movements <- data.frame(from = c(8,8,8), to = c(9, 30, 41),
                            Movement = c("Overshoot", 
                                         "Home", "Loss"))
TUC_wild_transported_temp_move_prob_array <- estimate_temp_effect_SRW_T(origin_select = "Tucannon River", movements = TUC_movements)
TUC_hatchery_transported_temp_move_prob_array <- estimate_temp_effect_SRH_T(origin_select = "Tucannon River", movements = TUC_movements)
TUC_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, rear = "wild", origin_select = "Tucannon River")
TUC_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, rear = "hatchery", origin_select = "Tucannon River")

TUC_wild_not_transported_temp_move_prob_array <- estimate_temp_effect_SRW_NT(origin_select = "Tucannon River", movements = TUC_movements)
TUC_hatchery_not_transported_temp_move_prob_array <- estimate_temp_effect_SRH_NT(origin_select = "Tucannon River", movements = TUC_movements)
TUC_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, rear = "wild", origin_select = "Tucannon River")
TUC_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, rear = "hatchery", origin_select = "Tucannon River")

### Imnaha River ###
IMN_movements <- data.frame(from = c(9,9), to = c(37, 41),
                            Movement = c("Home", "Loss"))
IMN_wild_transported_temp_move_prob_array <- estimate_temp_effect_SRW_T(origin_select = "Imnaha River", movements = IMN_movements)
IMN_hatchery_transported_temp_move_prob_array <- estimate_temp_effect_SRH_T(origin_select = "Imnaha River", movements = IMN_movements)
IMN_wild_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_T_envir, rear = "wild", origin_select = "Imnaha River")
IMN_hatchery_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_T_envir, rear = "hatchery", origin_select = "Imnaha River")

IMN_wild_not_transported_temp_move_prob_array <- estimate_temp_effect_SRW_NT(origin_select = "Imnaha River", movements = IMN_movements)
IMN_hatchery_not_transported_temp_move_prob_array <- estimate_temp_effect_SRH_NT(origin_select = "Imnaha River", movements = IMN_movements)
IMN_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, rear = "wild", origin_select = "Imnaha River")
IMN_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, rear = "hatchery", origin_select = "Imnaha River")
...
ggsave(here::here("figures", "transport","temperature_effects", "JDR_wild_compare_movement_temp.png"), JDR_wild_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "UMA_wild_compare_movement_temp.png"), UMA_wild_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "UMA_hatchery_compare_movement_temp.png"), UMA_hatchery_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "YAK_wild_compare_movement_temp.png"), YAK_wild_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "WAWA_wild_compare_movement_temp.png"), WAWA_wild_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "WAWA_hatchery_compare_movement_temp.png"), WAWA_hatchery_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "WEN_wild_compare_movement_temp.png"), WEN_wild_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "WEN_hatchery_compare_movement_temp.png"), WEN_hatchery_compare_movement_temp, height = 8, width = 8)
...
ggsave(here::here("figures", "transport","temperature_effects", "ENT_wild_compare_movement_temp.png"), ENT_wild_compare_movement_temp, height = 8, width = 8)


### Tucannon River ###

# TUC wild, transported plot
TUC_wild_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Tucannon River",
                                                                                   wild_move_prob_array = TUC_wild_transported_temp_move_prob_array,
                                                                                   wild_covariate_experiences = TUC_wild_transported_covariate_experiences,
                                                                                   movements_evaluated = TUC_movements)

ggsave(here::here("figures", "transport","temperature_effects", "TUC_wild_transported_compare_movement_temp.png"), TUC_wild_transported_compare_movement_temp, height = 8, width = 8)

# TUC wild, not_transported plot
TUC_wild_not_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Tucannon River",
                                                                                               wild_move_prob_array = TUC_wild_not_transported_temp_move_prob_array,
                                                                                               wild_covariate_experiences = TUC_wild_not_transported_covariate_experiences,
                                                                                               movements_evaluated = TUC_movements)

ggsave(here::here("figures", "transport","temperature_effects", "TUC_wild_not_transported_compare_movement_temp.png"), TUC_wild_not_transported_compare_movement_temp, height = 8, width = 8)


# TUC hatchery, transported plot
TUC_hatchery_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Tucannon River",
                                                                                                   hatchery_move_prob_array = TUC_hatchery_transported_temp_move_prob_array,
                                                                                                   hatchery_covariate_experiences = TUC_hatchery_transported_covariate_experiences,
                                                                                                   movements_evaluated = TUC_movements)

ggsave(here::here("figures", "transport","temperature_effects", "TUC_hatchery_transported_compare_movement_temp.png"), TUC_hatchery_transported_compare_movement_temp, height = 8, width = 8)

# TUC hatchery, not_transported plot
TUC_hatchery_not_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Tucannon River",
                                                                                                       hatchery_move_prob_array = TUC_hatchery_not_transported_temp_move_prob_array,
                                                                                                       hatchery_covariate_experiences = TUC_hatchery_not_transported_covariate_experiences,
                                                                                                       movements_evaluated = TUC_movements)

ggsave(here::here("figures", "transport","temperature_effects", "TUC_hatchery_not_transported_compare_movement_temp.png"), TUC_hatchery_not_transported_compare_movement_temp, height = 8, width = 8)

### Imnaha River ###

# IMN wild, transported plot
IMN_wild_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Imnaha River",
                                                                                               wild_move_prob_array = IMN_wild_transported_temp_move_prob_array,
                                                                                               wild_covariate_experiences = IMN_wild_transported_covariate_experiences,
                                                                                               movements_evaluated = IMN_movements)

ggsave(here::here("figures", "transport","temperature_effects", "IMN_wild_transported_compare_movement_temp.png"), IMN_wild_transported_compare_movement_temp, height = 8, width = 8)

# IMN wild, not_transported plot
IMN_wild_not_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Imnaha River",
                                                                                                   wild_move_prob_array = IMN_wild_not_transported_temp_move_prob_array,
                                                                                                   wild_covariate_experiences = IMN_wild_not_transported_covariate_experiences,
                                                                                                   movements_evaluated = IMN_movements)

ggsave(here::here("figures", "transport","temperature_effects", "IMN_wild_not_transported_compare_movement_temp.png"), IMN_wild_not_transported_compare_movement_temp, height = 8, width = 8)


# IMN hatchery, transported plot
IMN_hatchery_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Imnaha River",
                                                                                                   hatchery_move_prob_array = IMN_hatchery_transported_temp_move_prob_array,
                                                                                                   hatchery_covariate_experiences = IMN_hatchery_transported_covariate_experiences,
                                                                                                   movements_evaluated = IMN_movements)

ggsave(here::here("figures", "transport","temperature_effects", "IMN_hatchery_transported_compare_movement_temp.png"), IMN_hatchery_transported_compare_movement_temp, height = 8, width = 8)

# IMN hatchery, not_transported plot
IMN_hatchery_not_transported_compare_movement_temp <- plot_compare_rear_temp_effect_multiple_movements(origin_select = "Imnaha River",
                                                                                                       hatchery_move_prob_array = IMN_hatchery_not_transported_temp_move_prob_array,
                                                                                                       hatchery_covariate_experiences = IMN_hatchery_not_transported_covariate_experiences,
                                                                                                       movements_evaluated = IMN_movements)

ggsave(here::here("figures", "transport","temperature_effects", "IMN_hatchery_not_transported_compare_movement_temp.png"), IMN_hatchery_not_transported_compare_movement_temp, height = 8, width = 8)
...
combined_movement_temp_plot <- ggarrange(JDR_wild_compare_movement_temp, UMA_wild_compare_movement_temp, UMA_hatchery_compare_movement_temp,
                                         ...
                                         TUC_wild_transported_compare_movement_temp, TUC_wild_not_transported_compare_movement_temp,
                                         TUC_hatchery_transported_compare_movement_temp, TUC_hatchery_not_transported_compare_movement_temp,
                                         temp_plot_legend_gg, nrow = 3, ncol = 5,
                                         labels = c("(A) JDR, Natural", "(B) UMA, Natural", 
                                                    ...
                                                    "(I) ENT, Natural",
                                                    "(J) TUC, Natural, Transported", "(K) TUC, Natural, Not Transported",
                                                    "(L) TUC, Hatchery, Transported", "(M) TUC, Hatchery, Not Transported"),
...
ggsave(here::here("figures", "transport","paper_figures", "Fig4_temperature_home_mainstem.png"), combined_movement_temp_plot, height = 12, width = 16)
...
# Snake River, wild, transported and not transported
# Note that since this movement is outside the DPS boundaries, it'll be shared across all origins
# so we can just choose one and it'll be the same for all
SR_wild_transported_temp_DES_move_prob_array <- estimate_temp_effect_SRW_T(origin_select = "Tucannon River", movements = des_movement)
SR_wild_not_transported_temp_DES_move_prob_array <- estimate_temp_effect_SRW_NT(origin_select = "Tucannon River", movements = des_movement)

# Snake River, hatchery, transported and not transported
# Note that since this movement is outside the DPS boundaries, it'll be shared across all origins
# so we can just choose one and it'll be the same for all
SR_hatchery_transported_temp_DES_move_prob_array <- estimate_temp_effect_SRH_T(origin_select = "Tucannon River", movements = des_movement)
SR_hatchery_not_transported_temp_DES_move_prob_array <- estimate_temp_effect_SRH_NT(origin_select = "Tucannon River", movements = des_movement)

### Step 2: Reformat predicted movement probabilities for plot ###
# little function to reformat movement prob arrays
reformat_DES_prob_array <- function(move_prob_array, origin_name, rear_type, transport_type){
  ...
  movement_df %>% 
    ...
    mutate(origin = origin_name,
           rear = rear_type,
           transport_history = transport_type) -> movement_df
...
JDR_natural_DES_move_long <- reformat_DES_prob_array(JDR_wild_temp_DES_move_prob_array, origin_name = "John Day River", rear_type = "natural",
                                                     transport_type = "not transported")
...
# Snake River, Wild, transported and not transported
SR_wild_transported_DES_move_long <- reformat_DES_prob_array(SR_wild_transported_temp_DES_move_prob_array, origin_name = "Snake River", rear_type = "natural",
                                                             transport_type = "transported")
SR_wild_not_transported_DES_move_long <- reformat_DES_prob_array(SR_wild_not_transported_temp_DES_move_prob_array, origin_name = "Snake River", rear_type = "natural",
                                                                 transport_type = "not transported")

# Snake River, Hatchery, transported and not transported
SR_hatchery_transported_DES_move_long <- reformat_DES_prob_array(SR_hatchery_transported_temp_DES_move_prob_array, origin_name = "Snake River", rear_type = "hatchery",
                                                                 transport_type = "transported")
SR_hatchery_not_transported_DES_move_long <- reformat_DES_prob_array(SR_hatchery_not_transported_temp_DES_move_prob_array, origin_name = "Snake River", rear_type = "hatchery",
                                                                     transport_type = "not transported")
...
JDR_natural_DES_move_long %>% 
  bind_rows(., UMA_natural_DES_move_long, YAK_natural_DES_move_long, WAWA_natural_DES_move_long, 
            ...
            SR_wild_transported_DES_move_long, SR_wild_not_transported_DES_move_long,
            SR_hatchery_transported_DES_move_long, SR_hatchery_not_transported_DES_move_long) -> combined_DES_move_long

# summarise into median and 95% CI
combined_DES_move_long %>% 
  group_by(temp_actual, origin, rear, transport_history) %>% 
  summarise(prob = quantile(prob, c(0.025, 0.5, 0.975)), q = c(0.025, 0.5, 0.975)) %>% 
  pivot_wider(names_from = q, values_from = prob) %>% 
  mutate(origin_rear = paste0(origin, ", ", rear),
         rear_transport_type = paste0(rear, ", ", transport_history)) -> combined_DES_move_summary
...
# Snake River, wild
# split by transported and not transported
...
TUC_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, 
                                                                            rear = "wild", origin_select = "Tucannon River")
...
ASO_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, 
                                                                            rear = "wild", origin_select = "Asotin Creek")
...
CLE_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, 
                                                                                rear = "wild", origin_select = "Clearwater River")
...
SAL_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, 
                                                                                rear = "wild", origin_select = "Salmon River")
...
GR_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, 
                                                                                rear = "wild", origin_select = "Grande Ronde River")
...
IMN_wild_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRW_NT_envir, 
                                                                                rear = "wild", origin_select = "Imnaha River")
...
TUC_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, 
                                                                                rear = "hatchery", origin_select = "Tucannon River")
...
CLE_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, 
                                                                                rear = "hatchery", origin_select = "Clearwater River")
...
SAL_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, 
                                                                                rear = "hatchery", origin_select = "Salmon River")
...
GR_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, 
                                                                               rear = "hatchery", origin_select = "Grande Ronde River")
...
IMN_hatchery_not_transported_covariate_experiences <- extract_covariate_experiences(envir = SRH_NT_envir, 
                                                                                rear = "hatchery", origin_select = "Imnaha River")
...
mutate(JDR_wild_covariate_experiences, origin_rear = "John Day River, natural",
  ...
  bind_rows( # MCW
    ...
    mutate(MET_hatchery_covariate_experiences, origin_rear = "Methow River, hatchery",
           transport_type = "not transported"),
    # SRW, transported
    mutate(TUC_wild_transported_covariate_experiences, origin_rear = "Tucannon River, natural",
           transport_type = "transported"), 
    mutate(ASO_wild_transported_covariate_experiences, origin_rear = "Asotin Creek, natural",
           transport_type = "transported"),
    mutate(CLE_wild_transported_covariate_experiences, origin_rear = "Clearwater River, natural",
           transport_type = "transported"), 
    mutate(SAL_wild_transported_covariate_experiences, origin_rear = "Salmon River, natural",
           transport_type = "transported"),
    mutate(GR_wild_transported_covariate_experiences, origin_rear = "Grande Ronde River, natural",
           transport_type = "transported"), 
    mutate(IMN_wild_transported_covariate_experiences, origin_rear = "Imnaha River, natural",
           transport_type = "transported"),
    # SRW, not transported
    mutate(TUC_wild_not_transported_covariate_experiences, origin_rear = "Tucannon River, natural",
           transport_type = "not transported"), 
    mutate(ASO_wild_not_transported_covariate_experiences, origin_rear = "Asotin Creek, natural",
           transport_type = "not transported"),
    mutate(CLE_wild_not_transported_covariate_experiences, origin_rear = "Clearwater River, natural",
           transport_type = "not transported"), 
    mutate(SAL_wild_not_transported_covariate_experiences, origin_rear = "Salmon River, natural",
           transport_type = "not transported"),
    mutate(GR_wild_not_transported_covariate_experiences, origin_rear = "Grande Ronde River, natural",
           transport_type = "not transported"), 
    mutate(IMN_wild_not_transported_covariate_experiences, origin_rear = "Imnaha River, natural",
           transport_type = "not transported"),
    # SRH, transported
    mutate(TUC_hatchery_transported_covariate_experiences, origin_rear = "Tucannon River, hatchery",
           transport_type = "transported"), 
    mutate(CLE_hatchery_transported_covariate_experiences, origin_rear = "Clearwater River, hatchery",
           transport_type = "transported"), 
    mutate(SAL_hatchery_transported_covariate_experiences, origin_rear = "Salmon River, hatchery",
           transport_type = "transported"),
    mutate(GR_hatchery_transported_covariate_experiences, origin_rear = "Grande Ronde River, hatchery",
           transport_type = "transported"), 
    mutate(IMN_hatchery_transported_covariate_experiences, origin_rear = "Imnaha River, hatchery",
           transport_type = "transported"),
    # SRH, not transported
    mutate(TUC_hatchery_not_transported_covariate_experiences, origin_rear = "Tucannon River, hatchery",
           transport_type = "not transported"), 
    mutate(CLE_hatchery_not_transported_covariate_experiences, origin_rear = "Clearwater River, hatchery",
           transport_type = "not transported"), 
    mutate(SAL_hatchery_not_transported_covariate_experiences, origin_rear = "Salmon River, hatchery",
           transport_type = "not transported"),
    mutate(GR_hatchery_not_transported_covariate_experiences, origin_rear = "Grande Ronde River, hatchery",
           transport_type = "not transported"), 
    mutate(IMN_hatchery_not_transported_covariate_experiences, origin_rear = "Imnaha River, hatchery",
           transport_type = "not transported")
...
# use factor levels for plotting
combined_DES_move_summary$rear_transport_type <- factor(combined_DES_move_summary$rear_transport_type, 
                                                        levels = c("natural, not transported", "hatchery, not transported",
                                                                   "natural, transported", "hatchery, transported"))


#### Deschutes river plot: 8-panel figure, with marginal histograms ####
...
plot_deschutes_temp_movement_onepop <- function(origin_rear_select = NA, DPS_select = NA){
  ...
  # create a plot that shows transported fish
  rear_transport_colors <- c("natural, not transported" = "#1f78b4", "hatchery, not transported" = "#b2df8a",
                             "natural, transported" = "#cab2d6",
                               "hatchery, transported" = "#e31a1c")
  
  covariate_data %>% 
    mutate(rear_transport_type = paste0(rear, ", ", transport_type)) -> covariate_data
  
  
  des_move_prob_plot <- ggplot(move_prob_data, aes(x = temp_actual, y = `0.5`, ymin = `0.025`, ymax = `0.975`, 
                                                              color = rear_transport_type, fill = rear_transport_type)) +
    geom_line() +
    geom_ribbon(alpha = 0.2, color = NA) +
    scale_y_continuous(lim = c(0,1)) +
    # scale_linetype_manual(values = c("natural" = 1, "hatchery" = 2), name = "Rearing Type") +
    scale_color_manual(values = rear_transport_colors, name = "Rearing/Transport Type") +
    scale_fill_manual(values = rear_transport_colors, name = "Rearing/Transport Type") +
    # common x-axis scale across all populations
    coord_cartesian(xlim = c(4, 22.5), expand = FALSE) +
    # xlab(expression(~"Temperature" ~ ("°C"))) +
    # ylab("Movement probability") +
  ...
  des_density_plot <- ggplot(covariate_data, 
                                       aes(temp_actual, fill = rear_transport_type))+
...
des_move_prob_plot_legend <- ggplot(subset(combined_DES_move_summary, origin_rear %in% c("Snake River, natural", "Snake River, hatchery")), aes(x = temp_actual, y = `0.5`, ymin = `0.025`, ymax = `0.975`, 
                                                 color = rear_transport_type, fill = rear_transport_type)) +
  geom_line() +
  geom_ribbon(alpha = 0.2, color = NA) +
  scale_y_continuous(lim = c(0,1)) +
  # scale_linetype_manual(values = c("natural" = 1, "hatchery" = 2), name = "Rearing Type") +
  scale_color_manual(values = rear_transport_colors, name = "Rearing/Transport Type",
                     labels = c("natural,\nnot transported", "hatchery,\nnot transported",
                                "natural,\ntransported", "hatchery,\ntransported" )) +
  scale_fill_manual(values = rear_transport_colors, name = "Rearing/Transport Type",
                    labels = c("natural,\nnot transported", "hatchery,\nnot transported",
                               "natural,\ntransported", "hatchery,\ntransported" )) +
  # common x-axis scale across all populations
...
ggsave(here::here("figures", "transport","paper_figures", "Fig5_deschutes_temp_movements.png"), 
