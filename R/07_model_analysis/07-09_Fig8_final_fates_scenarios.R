...
load("figures/final_fates_scenarios/simulation_runs/TUC_LGR_winterspill_homing.rda")
...
plot_ff_cov <- function(data, dam_name, dam_spill_column){
  ...
  plot <- ggplot(data, aes(x = eval(parse(text = dam_spill_column)), y = `0.5`, ymin = `0.025`, ymax = `0.975`, color = conditions,
    ...
    geom_linerange(aes(group = rear_condition), position=position_dodge(width=6)) +
    geom_point(aes(size = rear_type, group = rear_condition), position=position_dodge(width=6)) +
...
ggsave(here::here("figures", "paper_figures", "Fig8_FF_cov_combined_plot.png"), FF_cov_combined_plot, height = 12, width = 16)
