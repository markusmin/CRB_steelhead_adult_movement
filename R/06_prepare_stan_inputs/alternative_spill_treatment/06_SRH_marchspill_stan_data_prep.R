# 06 stan data prep: Snake River Hatchery (alternative spill treatment)
...
for (i in 1:nrow(subset(SR_origin_param_map, !(is.na(hatchery))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(SR_origin_param_map, !(is.na(hatchery)))$home_mainstem_state[i]){
      print(paste0("real<lower=0> sigma_yearxorigin", subset(SR_origin_param_map, !(is.na(hatchery)))$hatchery[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1))
    }
...
for (i in 1:nrow(subset(SR_origin_param_map, !(is.na(hatchery))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(SR_origin_param_map, !(is.na(hatchery)))$home_mainstem_state[i]){
      print(paste0("sigma_yearxorigin", subset(SR_origin_param_map, !(is.na(hatchery)))$hatchery[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1, " ~ cauchy(0,1)"))
    }
...
for (i in 1:nrow(subset(SR_origin_param_map, !(is.na(hatchery))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(SR_origin_param_map, !(is.na(hatchery)))$home_mainstem_state[i]){
      print(paste0("real sigma_yearxorigin", subset(SR_origin_param_map, !(is.na(hatchery)))$hatchery[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1, " = 0;"))
    }
...
n.ind <- dim(state_data)[3]
  ...
  # Same number of columns as origins: we need 1 column each origin
  ...
  for (i in 1:norigins){ # we need one parameter for each origin
  ...
  for (i in 1:norigins){ # loop is 1:norigins
    for (j in 1:nrow(snake_river_RE_year)){
      ...
      # Origin 1: Clearwater River: No post-overshoot fallback movements within the DPS
      # Origin 2: Grande Ronde River: No post-overshoot fallback movements within the DPS
      # Origin 3: Imnaha River: No post-overshoot fallback movements within the DPS
      # Origin 4: Salmon River: No post-overshoot fallback movements within the DPS
      # Origin 5: Tucannon River: 9-8 is a post-overshoot fallback movement within the DPS
      
      # this is for origins where 9-8 is a post-overshoot fallback movement
      if(snake_river_RE_year$row[j] == 9 & snake_river_RE_year$col[j] == 8 & i == 5){
  ...
  # We need one origin parameter for each origin
  for (i in 1:norigins){
    for (j in 1:nrow(snake_river_movements)){
      # Movements from mainstem to river mouth: store the two different versions
      if (snake_river_movements$mainstem_river_mouth_movement[j] == 1){
  ...
  # save(data, file = here::here("Stan", "snake_river_hatchery", "SRH_model_data.rda"))
  save(data, file = "Stan/alternative_spill_treatment/snake_river_hatchery/SRH_marchspill_model_data.rda")