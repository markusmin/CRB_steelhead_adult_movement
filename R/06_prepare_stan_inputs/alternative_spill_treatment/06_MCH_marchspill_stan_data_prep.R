# 06 stan data prep: Middle Columbia Hatchery (alternative spill treatment)
...
for (i in 1:nrow(subset(MC_origin_param_map, !(is.na(hatchery))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(MC_origin_param_map, !(is.na(hatchery)))$home_mainstem_state[i]){
      print(paste0("real<lower=0> sigma_yearxorigin", subset(MC_origin_param_map, !(is.na(hatchery)))$hatchery[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1))
    }
...
for (i in 1:nrow(subset(MC_origin_param_map, !(is.na(hatchery))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(MC_origin_param_map, !(is.na(hatchery)))$home_mainstem_state[i]){
      print(paste0("sigma_yearxorigin", subset(MC_origin_param_map, !(is.na(hatchery)))$hatchery[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1, " ~ cauchy(0,1)"))
    }
...
for (i in 1:nrow(subset(MC_origin_param_map, !(is.na(hatchery))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(MC_origin_param_map, !(is.na(hatchery)))$home_mainstem_state[i]){
      print(paste0("real sigma_yearxorigin", subset(MC_origin_param_map, !(is.na(hatchery)))$hatchery[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1, " = 0;"))
    }
...
states_complete %>% 
  ...
  count(natal_origin) -> midcol_hatchery_origin_table

origins_to_drop <- subset(midcol_hatchery_origin_table, n < 350)$natal_origin
...
# We have 2 origins - so this is going to be two columns, for one column for intercept and then one column for offsets from one origin
cat_X_mat_actual <- matrix(0, nrow = n.ind, ncol = 2)
...
temp_X_mat_actual <- matrix(0, nrow = n.ind, ncol = 3)
...
for (i in 1:2){ # since we only have two origins for hatchery
...
for (i in 1:2){ # since we have two origins for MCH
  for (j in 1:nrow(middle_columbia_tempxorigin)){
    # Movements from mainstem to river mouth: store the two different versions
    if (middle_columbia_tempxorigin$mainstem_river_mouth_movement[j] == 1){
...
# There are two origins for MCH, so we need two parameters
for (i in 1:2){
  for (j in 1:nrow(middle_columbia_movements)){
    # Movements from mainstem to river mouth: store the two different versions
    if (middle_columbia_movements$mainstem_river_mouth_movement[j] == 1){
...
save(data, file = "Stan/alternative_spill_treatment/middle_columbia_hatchery/MCH_marchspill_model_data.rda")
