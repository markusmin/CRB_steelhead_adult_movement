# 06 stan data prep: Middle Columbia Wild (alternative spill treatment)
# Alternative spill treatment: Treat spill days as only those in March, rather than across January, February, and March
...
subset(origin_param_map, natal_origin %in% c("Deschutes River", "John Day River",
                                             "Fifteenmile Creek", "Umatilla River",
                                             "Yakima River", "Walla Walla River")) -> MC_origin_param_map

MC_origin_param_map$home_mainstem_state <- c(2,2,2,2,3,3)

mainstem_states <- 1:9
DPS_mainstem_states <- 1:4

# find where the parameter is declared
for (i in 1:nrow(subset(MC_origin_param_map, !(is.na(wild))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(MC_origin_param_map, !(is.na(wild)))$home_mainstem_state[i]){
      print(paste0("real<lower=0> sigma_yearxorigin", subset(MC_origin_param_map, !(is.na(wild)))$wild[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1))
    }
...
for (i in 1:nrow(subset(MC_origin_param_map, !(is.na(wild))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(MC_origin_param_map, !(is.na(wild)))$home_mainstem_state[i]){
      print(paste0("sigma_yearxorigin", subset(MC_origin_param_map, !(is.na(wild)))$wild[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1, " ~ cauchy(0,1)"))
    }
...
for (i in 1:nrow(subset(MC_origin_param_map, !(is.na(wild))))){
  for (j in 1:length(DPS_mainstem_states)){
    if(DPS_mainstem_states[j] > subset(MC_origin_param_map, !(is.na(wild)))$home_mainstem_state[i]){
      print(paste0("real sigma_yearxorigin", subset(MC_origin_param_map, !(is.na(wild)))$wild[i], "_vector_", DPS_mainstem_states[j], "_", DPS_mainstem_states[j]-1, " = 0;"))
    }
...
states_complete %>% 
  ...
  count(natal_origin) -> midcol_wild_origin_table

origins_to_drop <- subset(midcol_wild_origin_table, n < 350)$natal_origin
...
tag_code_metadata <- read.csv("Data/covariate_data/tag_code_metadata.csv")
# keep only the fish that are in the dataset
# This keeps only wild origin fish
tag_code_metadata <- subset(tag_code_metadata, tag_code %in% states_complete$tag_code)
...
n.ind <- dim(state_data)[3]
  ...
  # Same number of columns as origins: we need 1 column for intercept and norigins - 1 columns for origin offsets
  ...
  # Set winter_spill_days to only be March spill days
  mar_spill_days -> winter_spill_days
  ...
  post_overshoot_combos_MC <- list(c("Deschutes_River", "Fifteenmile_Creek",
                                     "John_Day_River", "Umatilla_River"),
                                   c("Deschutes_River", "Fifteenmile_Creek",
                                     "John_Day_River", "Umatilla_River",
                                     "Walla_Walla_River", "Yakima_River"),
                                   c("Deschutes_River", "Fifteenmile_Creek",
                                     "John_Day_River", "Umatilla_River",
                                     "Walla_Walla_River", "Yakima_River"),
                                   c("Deschutes_River", "Fifteenmile_Creek",
  ...
  names(post_overshoot_combos_MC) <- c("mainstem, MCN to ICH or PRA", "mainstem, PRA to RIS",
  ...
  winter_post_overshoot_vector <- post_overshoot_vector(states_complete = states_complete_noloss, 
                                                     post_overshoot_combos = post_overshoot_combos_MC,
                                    month_numeric = c(3))
  ...
  middle_columbia_movements %>% 
    ...
    mutate(row_col = paste0(row, "_", col)) -> middle_columbia_tempxorigin
  ...
  # possible post-overshoot fallback movements for middle columbia DPS fish: states 3-9 to the state previous
  
  fallback_movements %>% 
    filter(row %in% 3:9) -> post_overshoot_fallback_movements
  ...
  # basically, just remove all middle_columbia_movements from the b0_matrix_names
  
  middle_columbia_movements %>% 
    mutate(movement_abbrev = paste0(row, "_", col)) -> middle_columbia_movements
  
  b0_matrix_names %>% 
    mutate(movement_abbrev = paste0(row, "_", col)) %>% 
    filter(!(movement_abbrev %in% middle_columbia_movements$movement_abbrev)) -> b0_matrix_names
  ...
  btemp_matrix_names %>% 
    mutate(row_col = paste0(row, "_", col)) %>% 
    filter(!(row_col %in% middle_columbia_tempxorigin$row_col)) -> btemp_matrix_names
  ...
  for (i in 1:norigins){ # loop goes 1:norigins
    for (j in 1:nrow(middle_columbia_tempxorigin)){
      
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_tempxorigin$mainstem_river_mouth_movement[j] == 1){
        cat("real ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_DE", ";", "\n", sep = "")
        cat("real ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_NDE", ";", "\n", sep = "")
        cat("real ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_DE", ";", "\n", sep = "")
        cat("real ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_NDE", ";", "\n", sep = "")
      }
      
      # If it's a within tributary movement - we don't want it
      else if (middle_columbia_tempxorigin$within_trib_movement[j] == 1){
        # do nothing!
      }
      # If it's a movement from mainstem to upstream - we also don't want it
      else if (middle_columbia_tempxorigin$mainstem_upstream_movement[j] == 1){
        # do nothing!
      } 
      # If we move from the river mouth state to the mainstem, we want a parameter - this is captured in the below
      # BUT - if it's a movement from the upstream state to the mainstem, we don't want a parameter.
      # these transitions will get the same parameters as the river mouth to mainstem
      else if (middle_columbia_tempxorigin$upstream_mainstem_movement[j] == 1){
        # do nothing!
      } else {
        # Finally - if it's any other movement, just treat it like normal!
        # If it's not, print just the one version
        cat("real ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], ";", "\n", sep = "")
        cat("real ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], ";", "\n", sep = "")
      }
  ...
  middle_columbia_movements %>% 
    filter(!(row>=10)) %>% 
    mutate(row_col = paste0(row, "_", col)) -> middle_columbia_RE_year
  ...
  for (i in 1:norigins){ # one parameter per origin
    for (j in 1:nrow(middle_columbia_RE_year)){
      ...
      if (middle_columbia_RE_year$mainstem_river_mouth_movement[j] == 1){
        cat("vector[nyears] ", "byearxorigin", i, "_raw_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE", ";", "\n", sep = "")
        cat("vector[nyears] ", "byearxorigin", i, "_raw_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_NDE", ";", "\n", sep = "")
      }
      
      # If it's a within tributary movement - we don't want it
      else if (middle_columbia_RE_year$within_trib_movement[j] == 1){
        # do nothing!
      }
      ...
      else if (middle_columbia_RE_year$mainstem_upstream_movement[j] == 1){
        # do nothing!
      } 
      ...
      else if (middle_columbia_RE_year$upstream_mainstem_movement[j] == 1){
        # do nothing!
      } else {
        # Finally - if it's any other movement, just treat it like normal!
        # If it's not, print just the one version
        cat("vector[nyears] ", "byearxorigin", i, "_raw_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ";", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # loop is 1:norigins
    for (j in 1:nrow(middle_columbia_RE_year)){
      ...
      # Origin 1: Deschutes River: 4-3 and 3-2 are both post-overshoot fallback movements within the DPS
      # Origin 2: Fifteenmile Creek: 4-3 and 3-2 are both post-overshoot fallback movements within the DPS
      # Origin 3: John Day River: 4-3 and 3-2 are both post-overshoot fallback movements within the DPS
      # Origin 4: Umatilla River: 4-3 and 3-2 are both post-overshoot fallback movements within the DPS
      # Origin 5: Walla Walla River: 4-3 is a post-overshoot fallback movement within the DPS
      # Origin 6: Yakima River: 4-3 is a post-overshoot fallback movement within the DPS
      
      # this is for origins where 3-2 is a post-overshoot fallback movement
      if(middle_columbia_RE_year$row[j] == 3 & middle_columbia_RE_year$col[j] == 2 & i %in% c(1,2,3,4)){
        cat("// real<lower=0> ", "sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE", ";", "\n", sep = "")
      } 
      
      # this is for origins where 4-3 is a post-overshoot fallback movement (which is all of them)
      ...
      # these transitions will get the same parameters as the river mouth to mainstem
        ...
        # Finally - if it's any other movement, just treat it like normal!
        # If it's not, print just the one version
        cat("real<lower=0> ", "sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ";", "\n", sep = "")
      }
  ...
  for (i in 1:nrow(middle_columbia_movements)){
    cat("b0_vector_DE[", parameter_indices_matrix[middle_columbia_movements$row[i], middle_columbia_movements$col[i]], "]", " = 0;", "\n", sep = "")
    cat("b0_vector_NDE[", parameter_indices_matrix[middle_columbia_movements$row[i], middle_columbia_movements$col[i]], "]", " = 0;", "\n", sep = "")
  }
  ...
  for (i in 1:norigins){ # loop is 1:norigins
    for (j in 1:nrow(middle_columbia_RE_year)){
      ...
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_RE_year$mainstem_river_mouth_movement[j] == 1){
        cat("byearxorigin", i, "_raw_parameters_matrix_DE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE);", "\n", sep = "")
        cat("byearxorigin", i, "_raw_parameters_matrix_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_NDE);", "\n", sep = "")
      }
      ...
      # For DE - do nothing - it'll just keep the zero from before
        ...
        cat("byearxorigin", i, "_raw_parameters_matrix_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j-1], "_", middle_columbia_RE_year$col[j-1], "_NDE);", "\n", sep = "")
      } 
      ...
      # These should not be in the movement df though, so it shouldn't matter.
        ...
        # If it's not, print just the one version
        cat("byearxorigin", i, "_raw_parameters_matrix_DE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ");", "\n", sep = "")
        cat("byearxorigin", i, "_raw_parameters_matrix_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ");", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # loop is 1:norigins
    for (j in 1:nrow(middle_columbia_RE_year)){
      ...
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_RE_year$mainstem_river_mouth_movement[j] == 1){
        cat("byearxorigin", i, "_actual_parameters_matrix_DE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE * sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE);", "\n", sep = "")
        cat("byearxorigin", i, "_actual_parameters_matrix_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_NDE * sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_NDE);", "\n", sep = "")
      }
      ...
      # For DE - do nothing - it'll just keep the zero from before
        ...
        cat("byearxorigin", i, "_actual_parameters_matrix_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j-1], "_", middle_columbia_RE_year$col[j-1], "_NDE * sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j-1], "_", middle_columbia_RE_year$col[j-1], "_NDE);", "\n", sep = "")
      } 
      # For year effects, there is no RE of year when moving from a tributary to the mainstem. So we will not do anything here.
      # These should not be in the movement df though, so it shouldn't matter.
      else if (middle_columbia_RE_year$upstream_mainstem_movement[j] == 1){
        ...
        # If it's not, print just the one version
        cat("byearxorigin", i, "_actual_parameters_matrix_DE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], " * sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ");", "\n", sep = "")
        cat("byearxorigin", i, "_actual_parameters_matrix_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], ", ] = to_array_1d(byearxorigin", i, "_raw_vector_",
            middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], " * sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ");", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # 1:norigins
    for (j in 1:nrow(middle_columbia_RE_year)){
      ...
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_RE_year$mainstem_river_mouth_movement[j] == 1){
        cat("sigma_yearxorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], 
            "] = sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE;", "\n", sep = "")
        cat("sigma_yearxorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], 
            "] = sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_NDE;", "\n", sep = "")
      }
      ...
      # For DE - do nothing - it'll just keep the zero from before
        ...
        cat("sigma_yearxorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], 
            "] = sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j-1], "_", middle_columbia_RE_year$col[j-1], "_NDE;", "\n", sep = "")
      } 
      # For year effects, there is no RE of year when moving from a tributary to the mainstem. So we will not do anything here.
      # These should not be in the movement df though, so it shouldn't matter.
      else if (middle_columbia_RE_year$upstream_mainstem_movement[j] == 1){
        ...
        # If it's not, print just the one version
        cat("sigma_yearxorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], 
            "] = sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ";", "\n", sep = "")
        cat("sigma_yearxorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_RE_year$row[j], middle_columbia_RE_year$col[j]], 
            "] = sigma_yearxorigin", i, "_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], ";", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # one per origin
    for (j in 1:nrow(middle_columbia_tempxorigin)){
      # Movements from mainstem to river mouth: store the two different versions
      if (middle_columbia_tempxorigin$mainstem_river_mouth_movement[j] == 1){
        cat("btemp0xorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_DE", ";", "\n", sep = "")
        cat("btemp0xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_NDE", ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_DE", ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_NDE", ";", "\n", sep = "")
      ...
      # For DE - we don't care about this, because we're removing the upstream states
        ...
        cat("btemp0xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j-1], "_", middle_columbia_tempxorigin$col[j-1], "_NDE", ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j-1], "_", middle_columbia_tempxorigin$col[j-1], "_NDE", ";", "\n", sep = "")
        
      }
      # If it's a movement from the upstream state back to the mainstem, give it the same parameter as movement from the river mouth to the upstream
      # And note - that these are not DE or NDE parameters
      else if (middle_columbia_tempxorigin$upstream_mainstem_movement[j] == 1){
        cat("btemp0xorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j-1], "_", middle_columbia_tempxorigin$col[j],  ";", "\n", sep = "")
        cat("btemp0xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j-1], "_", middle_columbia_tempxorigin$col[j], ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j-1], "_", middle_columbia_tempxorigin$col[j],  ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j-1], "_", middle_columbia_tempxorigin$col[j], ";", "\n", sep = "")
        # If it's not, store the same parameter in both matrices
      } else {
        cat("btemp0xorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], ";", "\n", sep = "")
        cat("btemp0xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j],  ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], ";", "\n", sep = "")
        cat("btemp1xorigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_tempxorigin$row[j], middle_columbia_tempxorigin$col[j]], "]", " = ", "btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j],  ";", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){
    for (j in 1:nrow(middle_columbia_movements)){
      # Movements from mainstem to river mouth: store the two different versions
      if (middle_columbia_movements$mainstem_river_mouth_movement[j] == 1){
        cat("borigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_DE", ";", "\n", sep = "")
        cat("borigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_NDE", ";", "\n", sep = "")
      ...
      # For DE - we don't care about this, because we're removing the upstream states
        ...
        cat("borigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j-1], "_NDE", ";", "\n", sep = "")
      ...
      # And note - that these are not DE or NDE parameters
        ...
        # cat("borigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j],  ";", "\n", sep = "")
        cat("borigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j], ";", "\n", sep = "")
        # If it's not, store the same parameter in both matrices
      } else {
        cat("borigin", i, "_vector_DE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], ";", "\n", sep = "")
        cat("borigin", i, "_vector_NDE[", parameter_indices_matrix[middle_columbia_movements$row[j], middle_columbia_movements$col[j]], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j],  ";", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # one per origin
    for (j in 1:nrow(middle_columbia_tempxorigin)){
      ...
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_tempxorigin$mainstem_river_mouth_movement[j] == 1){
        cat("btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_DE", " ~ normal(0,5);", "\n", sep = "")
        cat("btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_NDE", " ~ normal(0,5);", "\n", sep = "")
        cat("btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_DE", " ~ normal(0,5);", "\n", sep = "")
        cat("btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], "_NDE", " ~ normal(0,5);", "\n", sep = "")
      }
      ...
      # these transitions will get the same parameters as the river mouth to mainstem
        ...
        # Finally - if it's any other movement, just treat it like normal!
        # If it's not, print just the one version
        cat("btemp0xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], " ~ normal(0,5);", "\n", sep = "")
        cat("btemp1xorigin", i, "_matrix_", middle_columbia_tempxorigin$row[j], "_", middle_columbia_tempxorigin$col[j], " ~ normal(0,5);", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # one for each origin
    for (j in 1:nrow(middle_columbia_movements)){
      ...
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_movements$mainstem_river_mouth_movement[j] == 1){
        cat("borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_DE", " ~ normal(0,5);", "\n", sep = "")
        cat("borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_NDE", " ~ normal(0,5);", "\n", sep = "")
      }
      ...
      # these transitions will get the same parameters as the river mouth to mainstem
        ...
        # Finally - if it's any other movement, just treat it like normal!
        # If it's not, print just the one version
        cat("borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], " ~ normal(0,5);", "\n", sep = "")
      }
  ...
  for (i in 1:norigins){ # one per origin
    for (j in 1:nrow(middle_columbia_RE_year)){
      ...
      # Movements from mainstem to river mouth: print two versions
      if (middle_columbia_RE_year$mainstem_river_mouth_movement[j] == 1){
        cat("byearxorigin", i, "_raw_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_DE", " ~ normal(0,1);", "\n", sep = "")
        cat("byearxorigin", i, "_raw_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], "_NDE", " ~ normal(0,1);", "\n", sep = "")
      }
      ...
      # There are no year effects for movements from tributaries, but this doesn't matter because it should have been removed earlier
        ...
        # Finally - if it's any other movement, just treat it like normal!
        # If it's not, print just the one version
        cat("byearxorigin", i, "_raw_vector_", middle_columbia_RE_year$row[j], "_", middle_columbia_RE_year$col[j], " ~ normal(0,1);", "\n", sep = "")
      }
  ...
  save(data, file = "Stan/alternative_spill_treatment/middle_columbia_wild/MCW_marchspill_model_data.rda")
